...
- 今日决策卡拆成：**今日顺序**（14:20 提醒 → 14:30 名单 → ①核心 ETF ②到期卖 ③缺口买）→ 核心 ETF 块 → **卫星仓**（持有 / body 到期卖 / 缺口买）→ A 股线（股票篮，pick≠STOCK 时闲置）。占用真值 = Watchlist 最多 4 槽；引擎 `openPositions` 只作对照（卡片写「C4 占用对照，不是交易铃」）。独立 paper 簿 `source=twin_star`（17:43，body=3 / −5%），不走 S-3 C4 / 金字塔 / 60 日持有
- **卫星何时卖**：按 Watchlist `entryDate` 数交易日，第 3 日收盘卖（`body=3`）。到期日与「已持 n/3」写在每只旁边；到日或现价跌破保护止损会改成卖出
- **保护止损（券商条件单）**：成本 × 0.95（−5%）。这是实盘旁路，**不是** S-gap 回测腿。每只可「复制止损」，顶部可「复制止损单」一次贴到券商。没有入场日/成本时会提示补录，无法算到期或止损价
- **提醒铃铛**按 Settings 策略拆开：`twin_star` 只报卫星 body3 / 保护止损 / 核心择强；STOCK 日 leftover 篮内才报 S-3 金字塔。**不**报港股缺票对账。面板分「今日交易 / 系统 / 策略体检」；滚动 OOS、S-3 纸面对账只在回测页标「单轨对照」，不当成下单。`GET /api/notifications?mode=`
