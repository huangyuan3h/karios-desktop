...
## TIP-014 — 环境×买入风格适配（2026-08-14 立）

**状态**：[ ]（Phase 1 实验进行中）
**来源**：用户真实执行节奏披露——A股/港股 14:00-15:00 随机时刻按信号买入（非收盘价）。
**背景**：02099 案例（8/12 收盘 202.8 入场 → 8/13 -6.4% 止损 → 8/14 业绩利好 +14.6%）
暴露「收盘价入场」代价；且 `last_hour_low` 回测假设（能买在尾盘低点）不可执行——
真实买点是在 2-3 点随机时刻。因此不优化入场价，而是优化**买入方向**：
不同市场环境下该买什么类型的票。

**核心假设（用户直觉，待回测验证）**：
- 电风扇行情（板块快速轮动）→ 买**短期调整大**的强势股（回调低吸）有收益
- 主升行情（主线明确持续）→ 买**龙头**（追强）有收益
- 弱势/恐慌 → 纪律防守（现有闸门已覆盖）

**Phase 1 — 实验（回测引擎内做，不碰 live）**：
1. 环境标签（每日，用 `market_cn_sentiment_daily` 6 字段）：
   - 主升：涨跌比 > 2 + 涨停溢价 > 0 + riskMode ∈ {hot, normal}
   - 电风扇：涨跌比 ∈ [0.5, 1.5] + 主线 top3 每日切换率高（industry-mainline）
   - 弱势：riskMode ∈ {extreme_caution, no_new_positions}
   - 数据窗口限制：sentiment 表 2026-01 起 → 环境标签仅 train/valid 窗生效，OOS2 回退中性
2. 买入风格（`BacktestConfig.entry_style`）：
   - momentum：RS > 80% + 近 5 日涨幅为正 + 主线内（追强/龙头）
   - dip：RS > 80% 但近 5 日跌幅 > 5%（强势股回调低吸）
   - score：现状（无额外条件）
3. 交叉矩阵：环境（3）× 风格（3）= 9 组 × 三窗（OOS2/train/valid）walk-forward
4. 验收：两组收益差必须三窗一致（单一窗好看 = 过拟合，todo §19 铁律）

**Phase 1 初步结论（2026-08-14）**：
1. **主升日追强有效** ✅：valid 窗 uptrend 日 37 笔，momentum 组 32 笔 avg +27.7%（验证用户直觉"主升买龙头"）
2. **电风扇样本不足** ⚠️：RS≥0.8 且 5日跌幅≥5% 的候选太少（valid 仅 6 笔），"电风扇买回调"无法验证——**待获取更多分钟线/更长历史后重试**
3. **neutral 日（有数据但非主升非风扇）16 笔全亏 avg -6.1%** 🎯：非主升非风扇的杂日可能根本不该开仓——比风格切换更重要，优先验证并固化
...
- 归因：HK 动量股极少 5 日回调（momentum 占 OOS2 273/344、valid 116/122 笔）→ dip 分支形同虚设；regime 与收益无稳定关系（valid Strong 日 -0.72% 亏但 OOS2 Strong 日 +0.53% 赚）
- 任何风格过滤（auto 默认映射 / 反向映射 / RS 收紧 0.7）都破坏 OOS2（-170~-181pt）→ **HK 线维持 entry_style=score（无风格过滤）**；`hk_style_map` 保留为实验参数（默认空 = 不用）
- 差异根因：CN 环境信息量足（4+ 信号），HK 只有 2 个指数红绿灯——环境感知需要足够的环境数据
