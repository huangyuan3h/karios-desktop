# 状态分桶（State-Bucket）算法真值【2026-08-31 定案冻结 · R12 双子星 (Twin-Star)】
...
| 结构 | **双子星 (Twin-Star) 50/50**：择强 trail8 核心 + S-gap 卫星（R12 冻结） |
...
### 3.0 冻结：双子星 50/50（R12 · `core_satellite_frozen_2026-08-31.json`）
...
| aligned | +189.4/12.6/2.54 | +125.6/13.3/3.31 | 0.01 | **+164.0/8.1/3.81** |

> \* valid 短窗 CAGR 年化失真（50/50 total +99.4%），看相对差异即可。

> **2026-08-31 v2 重固化**（`core_satellite_frozen_2026-08-31.json` status=v2）：aligned 初版（sat +132.8 / blend +168.4 / dd 6.4）固化时 `stock_dailybasic` 停更于 08-07，卫星最后 3 周被"无 mv 剔除"强制空仓——躲过了 08-10~08-28 实亏段（卫星 -6.3%）。OPT-057 补齐 mv 后同窗重跑为**真实值**：sat +126.3/dd13.3、blend **+164.0/dd8.1/sr3.81**；OOS2/train/valid/past_year（均止于 08-07 前）逐位不变。
>
> **长窗扩展验证（数据完整起点起，2024-08-01~2026-08-31）**：核心 +198.1/18.2/1.54 · 卫星 +364.4/22.1/2.66 · **双子星 +293.2/18.2/2.77**（corr 0.044）——两年窗夏普 2.77 vs 核心单轨 1.54，卫星风险调整贡献显著；分段 2024H2 +61.1/sr3.63 · 2025 +48.3/sr2.35 · 2026YTD +66.9/sr2.90，无过拟合退化。
...
| 指标 | 择强单轨 trail8 | S-gap 卫星 | **双子星 50/50** |
|------|----------------|------------|---------------------|
| 总收益 | +190.7% | +126.3% | **+164.0%** |
| 最大回撤 | 12.6% | 13.3% | **8.1%** |
| sharpe | 2.54 | 3.31 | **3.81** |

> v2 重固化：初版卫星 +132.8/dd9.2/sr3.67、blend +168.4/dd6.4/sr3.93 系 mv 停更缺陷值（卫星被强制空仓躲过 08-10~08-28 亏损段），见 §3.0 注。
...
| **双子星（R12 冻结）** | **+164.0%（同窗 total）** | **8.1** | **3.81** | 跨资产+A 股 |
...
| **aligned 2025-08-28~2026-08-28（去年）** | **+190.7%** | **+126.3%** | **+164.0%** | **8.1** | **3.81** |
| long2y 2024-08~2026-08（扩展） | +198.1% | +364.4% | +293.2% | 18.2 | 2.77 |

> 复现命令：卫星 `scripts/scout_state_pk_combo.py`（或逐段 `simulate_state_bucket` + `combo_nav`）；核心 `scripts/pick_strong_grid.py --batch E`；合成后对照 `core_satellite_frozen_2026-08-31.json`（status=v2：aligned 为 mv 修复后真实值，初版 +132.8/+168.4/dd6.4 系卫星被强制空仓的缺陷值，见 §3.0 注）。
...
- **内容**：核心 = live sleeve 当前目标（mom_compare+trail8 同源）；卫星 = 最新收盘（t-1）的 R-wide 闸 + S-gap 低波 33% 候选（至多 5 只）。
