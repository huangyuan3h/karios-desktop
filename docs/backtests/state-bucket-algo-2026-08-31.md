# 状态分桶（State-Bucket）算法真值【2026-09-01 定案冻结 · 机会双子星 (Opportunity Twin-Star)】

> **一句话**：**择强单轨（trail8）为主干，S-gap 卫星作"机会增强"**——卫星资金平时 100% 跟核心（资金利用率满），大盘宽度上行（R-wide 开闸）且候选**可执行**（T-1 收盘未涨停）时才把 50% 切片换成低波 33% 候选（持有 3 日）。
> **2026-09-01 涨停审计**：旧 50/50 双子星回测假设一字板开盘可成交（冻结窗 33% 入场买不进），卫星收益 80%+ 来自买不进的涨停票；剔除后旧混合 aligned 仅 +96.7%（**反而稀释核心**）。机会口径六窗全部 ≥ 核心单轨（aligned **+205.9%** vs 核心 +190.7%）。
>
...
| 结构 | **机会双子星 (Opportunity Twin-Star)**：择强 trail8 主干 + S-gap 卫星机会增强（卫星资金跟核心，开闸可买才切候选）· 2026-09-01 定案替代旧 50/50 |
...
### 3.0 冻结：机会双子星（2026-09-01 · `core_satellite_frozen_2026-08-31.json` status=FROZEN 机会双子星）
| 窗口 | 择强核心 | S-gap 卫星(可执行) | **机会双子星 total/dd/sr** | vs 核心 |
|------|---------|--------------------|---------------------------|---------|
| OOS2 | +17.8% | +111.4% | **+70.1 / 17.0 / 1.97** | +52.3pt |
| train | +40.7% | +35.3% | **+49.6 / 6.1 / 3.72** | +9.0pt |
| valid* | +139.1% | +10.3% | **+163.0 / 11.9 / 3.78** | +23.9pt |
| past_year | +181.2% | +45.0% | **+199.8 / 12.6 / 2.61** | +18.6pt |
| aligned | +190.7% | +25.4% | **+205.9 / 12.6 / 2.66** | +15.3pt |
| long2y | +198.1% | +152.4% | **+285.1 / 17.0 / 1.90** | +87.0pt |

> \* valid 短窗 CAGR 年化失真，看相对差异即可。
> 机会口径：`opp_ret = core_ret`（卫星无持仓日）`+ 0.5×(sat_ret − core_ret)`（卫星持仓日）；卫星引擎 `skip_t1_limit=True`（T-1 收盘涨停候选剔除）+ universe 排除 ST/BJ/退市。

**2026-09-01 涨停可成交审计（旧 50/50 双子星被替代的根因）**：
1. 冻结窗 671 笔卫星入场中 **一字板 24.1% + 涨停开盘 8.9% = 33% 实际买不进**（回测按 T 日开盘价成交 → 高估收益）
2. 可执行口径（剔除 T-1 涨停候选）卫星：aligned 126.3→**25.4%**、OOS2 228.7→111.4%（卫星收益 80%+ 来自买不进的涨停票）
3. 旧 50/50 混合（可执行口径）aligned 仅 **+96.7%** < 核心 +190.7% → 卫星稀释核心 → **机会口径六窗全面 ≥ 核心**，且资金利用率 100%（卫星闲置资金跟核心）
4. 卖出端审计：body=3 卖出日跌停封死仅 0.1%（667 笔 1 笔）→ 卖不出可忽略；核心腿 ETF 两年一字涨停/跌停 0~1 次（QDII 无涨跌幅限制）→ 单轨择强无买卖受阻问题
5. Universe 修正：旧引擎 `_load_rows` 未过滤 ST/BJ/退市（56.8 万行污染数据，BJ 涨跌幅 30%）→ 已修复

> 历史快照：旧 50/50 双子星（R12 2026-08-31 v2）aligned +164.0/dd8.1/sr3.81、长窗 +293.2/18.2/2.77 —— 均为"一字板可成交"口径的虚高值，已归档见下文 §3.2 注。
...
| 指标 | 择强单轨 trail8 | S-gap 卫星(可执行) | **机会双子星** |
|------|----------------|---------------------|---------------------|
| 总收益 | +190.7% | +25.4% | **+205.9%** |
| 最大回撤 | 12.6% | 11.9% | **12.6%** |
| sharpe | 2.54 | — | **2.66** |

> 机会双子星 = 核心单轨 + 卫星机会增强（卫星资金平时 100% 跟核心，开闸可买才切 50% 切片）。旧 50/50 双子星（2026-08-31 v2 冻结：+164.0/dd8.1/sr3.81）为"一字板可成交"虚高口径，**已被机会口径替代**。
...
| **机会双子星（2026-09-01 定案）** | **+205.9%（同窗 total）** | **12.6** | **2.66** | 跨资产+A 股 |
...
| **aligned 2025-08-28~2026-08-28（去年）** | **+190.7%** | **+25.4%** | **+205.9%** | **12.6** | **2.66** |
| long2y 2024-08~2026-08（扩展） | +198.1% | +152.4% | +285.1% | 17.0 | 1.90 |

> 复现命令（机会双子星）：核心 `scripts/pick_strong_grid.py --batch E`；卫星 `build_sgap_timeline(start, end, skip_t1_limit=True)`（T-1 涨停候选剔除 = 可执行口径）；合成 `build_twin_star_timeline(..., opportunity=True)`（卫星跟核心，开闸切候选）；对照 `core_satellite_frozen_2026-08-31.json`（status=机会双子星）。旧 50/50 数字（aligned +164.0/dd8.1）为"一字板可成交"虚高口径，仅作历史记录。

### 7.7 实盘执行映射（机会双子星）

- **主干（100% 资金）**：核心腿择强单轨——T-1 收盘算 mom60/MA200 → T 日满仓切换目标资产；ETF 峰值回撤 8% 日收盘切 REPO。**ETF 无买卖受阻问题**（QDII 无涨跌幅限制，两年一字板 0~1 次）。
- **机会增强（卫星）**：R-wide 开闸日（T-1 收盘判定）且候选**可执行**（T-1 未涨停封板）→ 卫星 50% 切片资金在 T 日开盘买入低波 33% 候选；持有 3 日收盘卖出；买不进（一字板）**放弃不追**，资金留在核心。
- **资金利用率**：卫星不参与的日子，其 50% 切片资金**全额跟核心**（不是闲置）。
- **执行顺序**：先按核心腿调仓（卖非 pick ETF/股票 → 补 pick 至 50%），卫星候选有开闸 + 可买再切。

### 7.8 14:30 前操作提醒（2026-08-31 · 机会双子星）

- **调度**：工作日 14:20 Asia/Shanghai（`twin_star_reminder_job`）→ webhook 事件 `twin_star_reminder`（按日去重）+ 通知中心 `twin_star` 类型（`_twin_star_notification`）；watchlist 今日操作卡内嵌卫星信号行（`GET /api/backtest/twin-star/action`）。
- **内容**：核心 = live sleeve 当前目标（mom_compare+trail8 同源）；卫星 = 最新收盘（t-1）的 R-wide 闸 + S-gap 低波 33% 候选（至多 5 只，**T-1 涨停已剔除**）。
- **数据依赖**：`stock_dailybasic`（mv）**已接入每日同步链路**（`stock_daily_basic_sync`，工作日 17:20，2026-08-31 补齐 08-10~08-31 缺口 88,677 行）；滞后时信号日仍会自动回退到最近有 mv 的收盘日并标注滞后（兜底）。
- **核心腿数据依赖**：5 只 ETF 日线（金/油/纳×2/债）由 `sleeve_etf_daily_sync`（工作日 17:25）每日增量同步——原全市场 `etf_daily_full` 为月频且限速失败，曾致 GOLD/BOND10 停更 7+ 天、mom_compare 失真（2026-08-31 修复 + 补齐 08-24~08-31）。
- **决策时序**：决策信号一律取 **t-1 收盘**（`_signal_closes` 显式剔除当日 bar；盘中 14:30 时 daily 表无当日 bar，旧实现 `closes[:-1]` 会回退到 t-2 造成 pick 落后一天——2026-08-31 修复）；trail8 用当日收盘价判断（与回测同口径，盘中不触发）。
- **注意**：卫星暂无实盘持仓簿，提醒只给"开闸 + 候选"；持仓到期（body=3）提醒待卫星簿落地后补。
