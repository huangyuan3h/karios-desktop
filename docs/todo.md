...
- [ ] **[P2] 回测探索方向池（2026-08-14 立 · 逐个验证价值后再决定是否做）**：
  - **D1 电风扇日细分（主线在 vs 无主线）**：dip 低吸在哪种电风扇更有效——数据已就绪，改动小
  - **D2 环境感知持有期（卖出侧空白）**：主升日买入缩短持有（吃主升段就跑）、电风扇日同理；max_hold=60 目前对所有环境一样
  - **D3 环境感知仓位**：主升日 12% / 电风扇日 8%（杠杆旋钮，夏普基本不变，优化收益/回撤比）
  - **D4 score_confirm_days（连续 N 天分数达标才买）**：过滤单日分数突刺（动量陷阱），引擎参数已有从未验证，30 分钟出结果
  - **D5 行业集中度上限 max_per_industry**：valid 电子占 48/55 笔——主线切换时是全仓风险点；上限 5/8/10 只，控回撤不损收益（风险控制向）
  - **D6 利润护城河 profit_trail 复核**：曾被排除（V7.0-03），低优先级
  - **D7 真实 5 分钟线验证尾盘执行**：需等东财风控解除（已验证可得，慢速分页拉取）
