...
- [ ] **[P2] 回测探索方向池（2026-08-14 立 · 逐个验证价值后再决定是否做）**：
  ...
  - **D3 环境感知仓位**：主升日 12% / 电风扇日 8%（杠杆旋钮，夏普基本不变，优化收益/回撤比）
  ...
  - **D6 利润护城河 profit_trail 复核**：曾被排除（V7.0-03），低优先级
  - **D7 真实分钟线验证尾盘执行**：**🔄 部分落地（2026-08-14）**：东财 push2his 被 IP 风控（502/断连，浏览器直连也拒=IP 级）→ 改用**腾讯分钟线接口**（web.ifzq.gtimg.cn `hkMinute/query` + `minute/query`，实测可用）：新表 `bar_minute`（alembic 0034）+ `minute_capture` service + scheduler job（16:35 盘后采集 open 持仓/自选 60 只当日 1 分钟线）；**数据从部署日起积累（腾讯分钟接口无历史，东财历史被风控）**；等积累 20+ 交易日后再验证 last_hour_low 近似 vs 真实尾盘价；东财风控解除后可补历史（慢速分页）
