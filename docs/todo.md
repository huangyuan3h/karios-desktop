...
- [ ] **[P2] 回测探索方向池（2026-08-14 立 · 逐个验证价值后再决定是否做）**：
  ...
  - **信号池 P1-P26（2026-08-15 立 · 用户 brainstorm 技术形态候选扩展）**：P1-P8 技术形态 + P9-P26 量价/基本面/风控候选——逐个验证「先有效 → 再查与 RS 重合 → 长窗+三窗+蒙特卡洛 → 有效才固化」；**计划与判定标准 → `backtests/experiments-planned.md`（§0b 速览 / §1 候选 / §4 顺序 / §5 已验证）**；已完成：
    ...
    - **信号池阶段总结（2026-08-15）**：**已验证 10 项全部拒收**（P1-P8 技术形态 8 项 + P12 波动率调整动量 + P16-ST 剔除）——绝对量技术形态 6 项无增量（与 A2 同向）、反转 2 项与候选集互斥、P12 砍 beta、P16 准入链已过滤；**完整结论 = S-3 的 alpha 只来自「RS 相对强弱 + 环境感知 + 纪律空仓」**；剩余待验证 14 项（P9/P10/P11/P13/P14/P15/P17/P18-P26）→ 下一候选建议 P11 行业中性 RS（详见 backtests/experiments-planned.md §4）
