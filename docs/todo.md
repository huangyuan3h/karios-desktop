...
> 给普通人的一页纸（2026-09-24 · 2 分钟版）：
> Karios 管的是家里钱中**博收益的那部分（卫星仓）**，每天告诉你买什么、卖什么、买多少。
> **Live 下单基线**叫**港湾（Harbor）**：S-3 股票核心 + H2 闲置现金停车场。
> **当前研究/展示/人工操作 canonical** 叫**稳健星舰 H2-a25**：卫星 4 槽×25% + 闲置现金 25% H2 ETF / 75% B3；不改 Harbor Live。
> H2-a25 独立回测 OOS2/train/valid/long **+238.4/+55.9/+3.2/+738.5**，long MDD **−8.4**、SR **3.50**；K1/K2/K4/K5 PASS，K3 风险未过（总状态 REJECT），不能写成全门 PASS。
> 旧机会双子星 4×12.5% 路径仍为历史 REJECT；旧 0pt a25 数字只作对照。
> 下一步：继续观察 H2-a25 的 K3 风险并完成人工账本对账；任何调参或进 Live 都必须另走预注册、paper 20 笔和用户风险授权。
...
| 港湾 Harbor | 🟢 **唯一产品基线**（2026-09-13 冻结 `harbor-p1-20260913`）= [S-3 核心 + 闲置现金 ETF 停车场](./modules/pick-strong-track.md)；三窗增量 +17.2/+13.0/+11.6、long +118.8（2026-09-14 clean 口径） |
| 旧双子星/择强单轨 | ⚪ 历史：旧 4×12.5% 路径被 B12 REJECT；稳健星舰 H2-a25 是独立研究/展示档，不接 Live |
...
- 旧卫星腿（旧双子星：14:30 名单 / C1 3% / 第 3 日 14:30 卖 / 4×12.5%）**已 REJECT 下线**；当前 H2-a25 是卫星 + 25% H2/75% B3 的独立研究/展示配方，K3 风险观察中，仍不接 Live。[H2-a25 档](backtests/stable/sat-h2-a25-2026-09-24.md)
...
| B18 | **游资解剖**（头部席位 × 封板质量 × 情绪周期 · 只读理解性） | tushare `top_inst` 席位级 2021+ ✅ · `stk_limit` ✅ · 全天 5min 事件集 2024–26 ✅；2021–23 全天待 baostock | **[done 2026-09-15 · 归档 [`archive/2026-09-15-hotmoney-anatomy.md`](archive/2026-09-15-hotmoney-anatomy.md)]**：E1 REJECT（剔一字后转负、拉萨对照 −2.23）· E2 REJECT（退出可见率 17.5%）· E4 REJECT（相位梯度干净但 d3 全负）· **E3 PASS**（封板质量分界真实，缺口归封板持有者）· E5 双 REJECT → **头部游资的钱 = 自己封板吃缺口；公开数据无可跟边缘**。⚠️ 含基期修正（qfq×raw→raw 重建，新增 `first-principles §二.8`） | 报告 `backtests/hotmoney/` 6 份 + SUMMARY 5 行；遗留：E3 long 窗（baostock 限流）；D3 快照 job 已上线（17:45 每日，回填 10 日）。**吸收（2026-09-15 · 零行为改动）**：AGENTS.md「Price basis」节 · `modules/README` 规则 10（拉萨警告 + 证据）· 代码注释（`trendok.py` / `state_bucket_track.py`）· `first-principles §二.1` 增补证据 · `designs/data-source-audit §8` 资产登记 |
| B19 | **缩量 / 地量后爆发**（长期下跌后地量见底？· 用户 2026-09-15 提出） | 库内即可：`daily` vol（个股 2010–2026，9.13M 过滤行）· `index_daily`（指数 2005+） | **[done 2026-09-15 · 预注册 [prereg](designs/volume-drought-prereg-2026-09-15.md) · REJECT/方向偏负]** 冻结定义（下跌态 + **250 日新低量**）：个股 10,986 事件 fwd60 超额 **−1.65%**（时代 −6.60/+0.13/−2.38/−1.66，仅 1/4 正）；剔最深跌幅十分位后 **−2.05**（非超跌换皮，独立更差）；指数仅 9–13 次/20 年、超额 −3%。**"地量后爆发"不成立：缩量阴跌 = 无量下跌→继续跌** | 档 [volume-drought](backtests/factors/volume-drought-2026-09-15.md)；换更松的"地量"定义属新假设须另起预注册（先验不利） |

**顺序**：A1/A2/A3 **[done]** → B1 CB **[done 线收口]** → **C1 打新 [done · 次要，有空再实现]**；**B2 利率 [done · REJECT（含预期差）]**；**B2 深层次三源 [done · 全 REJECT，大宗溢价最强生命体征]**；B2 指数调样（本地样本不足）暂缓；**B18 游资解剖 [done 2026-09-15 · 归档]**；**B19 缩量/地量 [done · REJECT/方向偏负，档 `backtests/factors/volume-drought-2026-09-15.md`]。**
...
| 2026-09-15 | P0-13 B18 游资解剖（席位级龙虎榜/封板质量/情绪相位 · 5 实验：E3 PASS 其余 REJECT · 含 qfq×raw 基期修正 → `first-principles §二.8`） | `archive/2026-09-15-hotmoney-anatomy.md` |
