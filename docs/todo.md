...
| 序 | 维度 | 为什么 | 当前 todo 域 |
|----|------|--------|--------------|
| 1 | **收益** | 立命之本 | §2 交易策略、§8 回测、**§22 多资产（黄金/原油/美股ETF）** |
...
4. **等待项**（用户拍板后激活）：B2 双模式、付费 API 矩阵、Mac mini 部署、L4（券商）

### 2026-08-23 战略转向：从单市场股票 → 多资产抗周期（用户拍板 · 收益域新 P0）

- **背景**：A 股 S-3 年化 17%（ realism 口径 66.6%/年 10%×10/DD8.4/83笔，S-3 已双年验证封闭），但 valid 是牛市切片（81.8% 胜率峰值）、长窗 2025 +1614% 为历史最强年，**刚爆发后沉寂概率大**（空仓期/震荡期已在 R5c WW 61% 中暴露）
- **决策**：不再在 S-3 上挤参（§19 封闭，B-T1 15× TrendOK 已证平台期无增益），**重心转 §22 多资产**：**1.黄金 2.原油 3.美股ETF（QDII）**——国内股票账户即可交易，与 A 股低相关，对冲沉寂期
- **与本节关系**：①②③ 仍并行（验证闭环不停），但人力 P0 切到 §22 G0-G2（2 周内闭环黄金/原油 research）；美股/加拿大远期（原等待项）由 §22 的 QDII ETF 513100/513500 先行落地，无需等 IBKR
...
| §22 多资产 | 🟡 **P0 主线 2026-08-23**：黄金/原油/美股ETF（国内ETF 518880/SC/513100）→ G0审计→G1单资产回测→G2联合（R5c扩展） |
...
- [ ] **[P2] 回测探索方向池（2026-08-14 立 · 逐个验证价值后再决定是否做）**：
  ...
  - **信号池 P1-P26（2026-08-15 立 · 用户 brainstorm 技术形态候选扩展）**：P1-P8 技术形态 + P9-P26 量价/基本面/风控候选——逐个验证「先有效 → 再查与 RS 重合 → 长窗+三窗+蒙特卡洛 → 有效才固化」；**计划与判定标准 → `backtests/experiments-planned.md`（§0b 速览 / §1 候选 / §4 顺序 / §5 已验证）**；已完成：
    ...
    - **P3 200 日均线**：**❌ 已拒收（2026-08-15）**——纯状态版 X0 拦截≈0 笔（候选全在 MA200 上方，信息已被 regime gate+RS 占满，收益差异是时序伪影）；缓冲版 X5/X10/X20 train 全劣化（-7.7~-18.3）+ 长窗 DD 45→65~108 恶化（缓冲=追高）；引擎 `ma200_min_pct` 保留默认 -1（关）
    - **P5 双均线金叉 / P6 三线多头**：**❌ 均已拒收（2026-08-15）**——P5（金叉事件）C3/C5/C10 全窗劣化 + 长窗灾难（金叉=滞后确认，追金叉=追高）；P6（三线状态）OOS2 -5.5 / train -6.3 劣化，valid +6.8 单窗不构成通过——**与 A2 预期完全一致**；引擎 `ma_cross_days`/`ma_aligned` 保留关闭
    - **P7 短线超卖反转 / P8 长阴次日反转**：**❌ 均已拒收（2026-08-15）**——P7（RSI<30）与 S-3 候选集互斥（RS 前 50% 票 RSI 不 <30 → valid/train 零样本），原设想的 weak 抄底已被 neutral_block（16/16 全亏）证否；P8（前日-5% 反转）valid 仅 2 笔 + 长窗 -38.9pt/DD72 劣化；**反转类（均值回归）与"只买强势票"哲学互斥**；引擎 `rsi_reversal_max`/`down_day_reversal_pct` 保留默认 0
    - **P16-ST ST 剔除**：**❌ 已拒收（2026-08-15）**——候选池真实存在 5% ST 票（33/673），但拦截 3145 次仅少 1 笔成交（准入链已隐性过滤）；三窗 0 劣化但长窗 -2.0pt + DD 45→51（**ST 摘帽/重组是真实 alpha**，一刀切剔除丢收益且回撤不降）；结论=S-3 的 score/RS/主线准入链已把 ST 尾部风险压到极低；引擎 `exclude_st` 保留默认关；质押/解禁/减持子项未测
...
## 22. 抗周期多资产：黄金 / 原油 / 美股 ETF（2026-08-23 立 · P0 收益域 · 用户拍板）

> **战略转向原因**：股票年化 17%（过去一年 66.6%/10%×10 口径，S-3 双年验证通过），但刚经历爆发段（valid 2026 牛市切片 win81.8%、长窗 2025 +1614%），**高位沉寂风险大**（下一段很可能是震荡/空仓期，R5c CN Weak 61% 已暴露）。用户核心目标=**赚钱/收益**，必须寻找**与 A 股低相关、国内可交易**的第二曲线。
>
> **用户拍板方向（按优先级）**：1. 黄金 2. 原油 3. 美股 ETF（QDII）——均为中国境内易交易标的。
> **本节 = 收益域新主线**；§19 S-3 已封闭（不再扫参），§22 接棒成为 2026Q4 唯一 P0。

### 22.1 国内可交易标的清单（已盘点 · 不新增账户前提）

| 资产 | 代表标的（国内交易所） | 交易路径 | 费率/杠杆/限制 | 数据源现状 | 备注 |
|------|------------------------|----------|----------------|------------|------|
| **黄金** | **518880 华安黄金ETF**（场内T+0）/ 518800 / 159934；**AU(T+D)** 上金所；纸黄金 | 场内ETF = 股票账户直接买（最易），费率 0.04%/年管理费，T+0，无杠杆 | tushare `fund_daily` 已有 518880 日线；上金所 AU 行情需补（tushare `gold` / 新浪/腾讯期货链） | **首选 518880**（流动性最大，日均 20-30亿，实测价≈现货×0.01） |
| **原油** | **SC 原油期货**（INE 上期能源）；**QDII：160216 国泰商品 / 162411 华宝油气 / 南方原油 501018 / 华安石油**；场内 **169101/160125** | SC 期货需期货账户（保证金 10% 杠杆，门槛 10万）；QDII ETF 股票账户直接买（无杠杆，T+0，溢价风险） | tushare `fut_daily` 有 SC 日线（当前 `macro_daily:119` 行仅 2026 年，需 stitch 连续合约）；QDII ETF 同 `fund_daily`（`159026.SZ` 仅 72 行 2026-04 起，受 200/min 限流） | **先做 QDII ETF（零门槛），SC 二阶段** |
| **美债/利率** | **511010 5年国债ETF / 511260 10年期 / 511130 30年期**（场内T+0）；**美债现货：US 10Y Yield**（FRED/tushare `bond_yc`）；**货基 GC001** | 债券ETF 股票账户直接买（年化 2-4%，波动 0.1-0.3%/日）；美债收益率需另建 `macro_daily` 序列 `US10Y`/`CN10Y` | 债券ETF `fund_daily` 当前 0 行（同受限流，需回填）；收益率无序列（`macro_daily source` 仅 fx/fut/index/tencent/eastmoney，需新增 `yield_daily`） | **5/10年期最易（波动最低，对冲股票 DD）** |
| **美股 ETF** | **513100 纳指ETF / 513500 标普500 / 159501 纳指100 / 513300 纳指科技**（QDII） | 股票账户直接买，T+0，溢价/限购风险（513100 常 2-5% 溢价，限额申购） | `fund_daily` 已有 513100 日线（实测 2026-08-10 已同步 2100+ ETF）；美股原指数需 yfinance/腾讯作 regime | **third-asset-sleeve 已用 513100**，链路复用 |
| 扩展 | 铜/豆粕/国债ETF 511010/511380/红利ETF 510880 | 同上 | 同上 | 低优先，二阶段再看 |

> **结论**：三类全部可用**现有股票账户 + fund_daily 表**先跑通（零新增开户），期货 SC 作增强。无需等新数据源即可启动 research。

### 22.2 与现有 todo 的收益对比（为什么现在做多资产 = ROI 最高）

| 现有 backlog | 预期收益 | 判定 | 处置 |
|--------------|----------|------|------|
| §19 S-3 参数空间 | 已扫 48+ 次，剩余 11 信号全拒收；B-T1 15× TrendOK 平台期 43%±4.5 无增益，score 65→90 也拒收 | **天花板已到**，再调=过拟合 | **封闭，不再投入**（仅季度复核） |
| §20.1 B-E3 HK 环境感知 / B-S1 套筒 / B-S2 SWAP / B-A1 R3 等 7 项业务债 | 单项 +3~11pt，但依赖新数据或双窗已证伪，叠加不确定 | 边际收益递减，属"修补" | P0 三项可并行但非主线 |
| §20.2 / §5 / §13 工程债（Tunnel、备份、限频、连接池等 14 项） | 稳定性↑，收益 0 | **0 收益**，纯成本 | 按需单开 OPT，不占 P0 人力 |
| §16 L4 券商自动下单 | 需 L3 验证闭环后，当前 C4 仅 2 笔不够 | 远期 | 暂缓 |
| §12 #9 付费 API 矩阵 / §14 MCP 等 | 决策成本，无直接收益 | 0 | 归档/低优 |
| **§22 黄金/原油/美股** | **新 beta 源，与 A 股相关性 <0.3（黄金-股票常 -0.1~0.2，原油~0.3，纳指~0.5 相对 A 股仍低），熊市/震荡期对冲，回撤熔断 DD89→40 的下一层防御** | **新曲线，空间最大** | **P0 主线** |

> **判断**：现有 backlog 里**没有**单项能像新资产那样开第二收益曲线；与其在 S-3 上挤 1-2pt，不如用 2-3 周把黄金/原油的**数据→回测→策略**跑通。
...
### 22.4 本周动作（2026-08-24 起 · 2 周内闭环 G0-G1）

1. **[P0] G0 数据审计脚本**（1 天）：`scripts/audit_commodity_etf_data.py` 扫 518880/南方原油/513100 三标的 2021 起完整性 + 缺口日历，输出 `docs/backtests/data-gap-commodity-2026-08-24.md`（同 2026-08-10 审计格式）
2. **[P0] G1-1 黄金回测**（3 天）：先跑 518880 单资产趋势（20/60/200 + ATR），三窗 + 长窗 + 与 A 股相关性；并行验证 SC 期货是否增益 > QDII
3. **[P0] G1-2 原油回测**（3 天，并行）：SC/QDII 双轨，Donchian 20/55 突破为基线
4. **[P1] G2-1 相关性矩阵**（G1 完成后 1 天）：四资产 2021-2026 日收益相关性 + 联合 NAV 初算，决定是否值得 G2-2 深入

> **退出条件**：若 G0 发现任一资产日线缺口 >10% 或 G1 三窗全不达标（夏普<1 / DD>50%），则该资产降级为 P2，仅保留 paper 观察，不入组合层。

### 22.5 不做清单（本节边界）

- ❌ 期货杠杆/期权（SC 保证金杠杆先不碰，只做 ETF 现货口径）
