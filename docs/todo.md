...
|----|------|
| §19 S-3 | ✅ 封闭（双年验证，不再扫参） |
| §22 多资产 | 🟡 **P0 主线** G0-G1 完成，G2 固化 `mom60+MA200` 三窗全过，剩周更+进实盘 |
| §8 回测 | ✅ 基线 `mp10` 三窗 `43.1/35.6/43.3`，C4 等 20 笔 |
| §4/5/6 | 按需 OPT，不占 P0 |

## 当前方向（验证+维护期）

- 验证闭环：`user_trades` 卖出按钮 → paper ≥20笔 → C4
- 系统健康：季度复核 + rolling OOS
- **P0 切 §22**：金→油→美股ETF (518880/513350/513100)，2周闭环
