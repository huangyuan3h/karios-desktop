...
- **禁止在不同字段重复同一观点**：「红绿灯/情绪」段落只写信号灯、广度、成交额与仓位建议；「热点书面分析 + 主线与资金流向」合起来写行业工作流、5D 资金与热点矩阵、主线与支线，不要再复述红绿灯结论；不要再次给出泛泛的仓位口号。
- 「市场环境要点提炼」**必须恰好 5 条** Markdown 列表（每条一行、以 "- " 开头）：从摘要与宏观信息中挑出**对 A 股与资产配置影响最大**的五条，合并同类项、去掉次要细节；不要整段照抄「市场环境摘要」原文句子。
...
- marketEnvironmentHighlights：**恰好 5 条**要点，每条一行，以 "- " 开头（Markdown 列表），只保留对 A 股与资产配置最关键的信息（指数、汇率、商品、外盘与地缘风险等）；条数不得多于或少于 5。
