...
describe('replica-gap', () => {
  ...
  it('flags stock basket while stock pick as info only', () => {
    const r = detectReplicaGaps({
      pick: 'STOCK',
      holdings: [
        { symbol: 'CN:600519', positionPct: 50 },
        { symbol: 'CN:000001', positionPct: 50 },
      ],
    });
    expect(r.verdict).toBe('aligned');
    expect(r.reasons.some((x) => x.id === 'stock_basket_vs_equal')).toBe(true);
  });
});
