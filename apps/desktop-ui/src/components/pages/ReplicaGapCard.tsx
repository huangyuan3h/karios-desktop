...
/**
 * Harbor core vs live book · attribution contrast.
 */
  ...
  return (
    <div className="rounded-lg border border-[var(--k-border)] bg-[var(--k-surface)] p-3">
      ...
      <p className="mb-2 text-[10px] text-[var(--k-muted)]">
        不问「今天 OIL 够不够 100%」，问：
        <strong>港湾钱从哪条腿来，你的仓位/成交接到了没有</strong>。
      ...
      {attrQ.isError ? (
        ...
        <p className="text-xs text-[var(--k-muted)]">暂无港湾归因</p>
      ) : (
        <div className="flex flex-col gap-3">
            ...
            <table className="w-full text-left text-[11px] tabular-nums">
              <thead className="sticky top-0 bg-[var(--k-surface)] text-[10px] text-[var(--k-muted)]">
                <tr>
                  ...
                  <th className="py-1 pr-2">港湾天数</th>
                  <th className="py-1 pr-2">港湾加法%</th>
          ...
          <p className="text-[10px] text-[var(--k-muted)]">
            {method?.detail}{' '}
            下方「涨跌归因」拆的是港湾核心腿；本卡把同一套 byPick
            与你的仓位/成交对齐。今日 pick 只作脚注——战术换仓看体检卡。
          </p>
        </div>
      )}
    </div>
