...
function rankChangeText(change: number | null | undefined): string {
        ...
        <table className="w-full border-collapse text-xs">
          <thead className="bg-[var(--k-surface-2)] text-[var(--k-muted)]">
            <tr className="text-left">
              ...
              {!compact ? <th className="px-2 py-2">RankΔ</th> : null}
              {!compact ? <th className="px-2 py-2">Signal</th> : null}
            ...
            {picks.length ? (
              picks.slice(0, 3).map((p, idx) => (
                <tr key={`${p.industryName}-${idx}`} className="border-t border-[var(--k-border)]">
                  ...
                  {!compact ? (
                    <td className="px-2 py-2 font-mono">{rankChangeText(p.rankChange)}</td>
                  ) : null}
                  {!compact ? (
                    <td className="px-2 py-2 font-mono">
                      {p.momentumSignal ? (
                        <span className="text-emerald-600 font-semibold">🚀 MOMENTUM</span>
                      ) : (
                        '—'
                      )}
                    </td>
