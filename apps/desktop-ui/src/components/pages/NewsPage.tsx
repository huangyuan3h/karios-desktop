...
import { RefreshCw, ExternalLink, Star, StarOff, Settings2 } from 'lucide-react';
...
export function NewsPage() {
  ...
  async function toggleSource(sourceId: string, enabled: boolean) {
    try {
      await apiPostJson(`/api/news/sources/${sourceId}`, { enabled });
      await refresh();
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    }
          ...
          <div className="space-y-2">
            {sources.map((src) => (
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-[var(--k-muted)]">{src.url}</div>
                </div>
                <div className="flex items-center gap-2">
                  {src.lastFetch ? (
                  ...
                  <Button
                    variant={src.enabled ? 'secondary' : 'ghost'}
                    size="sm"
            ...
            {sources.length === 0 ? (
              <div className="rounded-lg border border-dashed border-[var(--k-border)] px-3 py-6 text-center text-sm text-[var(--k-muted)]">
                No sources configured. Click "Add Defaults" to add common finance news sources.
              </div>
            ) : null}
          </div>
        </div>
