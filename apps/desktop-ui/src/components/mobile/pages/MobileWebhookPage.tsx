...
export function MobileWebhookPage() {
  ...
  const subs = useQuery({
    ...
    queryFn: () =>
      apiGetJson<{ subscriptions: WebhookSubscription[] }>('/api/webhook/subscriptions'),
  });
  ...
  const create = async () => {
    ...
    try {
      const res = await apiPostJson<{ subscription: WebhookSubscription }>(
            ...
            {list.map((s) => (
              <MobileCard key={s.id} className="p-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <StatusPill tone={s.enabled ? 'open' : 'closed'}>
                    {s.enabled ? '启用' : '停用'}
                  </StatusPill>
                </div>
      ...
      <MobileSheet open={creating} onClose={() => setCreating(false)} title="新建订阅">
        <div className="space-y-2.5">
          ...
          <MobileButton
            block
            onClick={() => void create()}
            disabled={!url.trim() || !selected.length}
          >
            创建
          </MobileButton>
        </div>
