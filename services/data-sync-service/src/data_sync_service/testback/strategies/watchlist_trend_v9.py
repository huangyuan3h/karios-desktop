...
class WatchlistTrendV9Strategy(BaseStrategy):
    """
    V9 strategy: balanced trend rotation with quality bias.

    Design upgrades vs V8:
    - Moderate candidate pool + liquidity-aware scoring.
    - Volatility contraction + volume confirmation for breakouts, but not overly strict.
    - Trend alignment (EMA20/60/120) to keep quality while allowing more signals.
    - Concentrated exposure in strong regimes with controlled rotation.
    """

    name = "watchlist_trend_v9"
    use_full_bars = True
    top_k = 70

    @classmethod
    def default_score_config(cls) -> ScoreConfig:
        return ScoreConfig(top_n=400, momentum_weight=1.0, volume_weight=0.35, amount_weight=0.20)
    ...
    def __init__(
        ...
        min_score_strong: float = 0.010,
        min_score_diverging: float = 0.006,
    ) -> None:
        ...
        self.long_window = max(self.slow_window + 20, int(long_window))
        ...
        self._history: Dict[str, Deque[Bar]] = defaultdict(lambda: deque(maxlen=380))
    ...
    def _score(self, closes: list[float], vols: list[float]) -> float:
        ...
        ret60 = (c0 / closes[-61] - 1.0) if len(closes) >= 61 and closes[-61] > 0 else 0.0
        ret150 = (c0 / closes[-151] - 1.0) if len(closes) >= 151 and closes[-151] > 0 else 0.0
        vol_boost = 0.0
        if len(vols) >= 20:
            avg_vol20 = sum(vols[-20:]) / 20.0
            if avg_vol20 > 0:
                vol_boost = (vols[-1] / avg_vol20) - 1.0
        return 0.60 * ret150 + 0.25 * ret60 + 0.15 * ret20 + 0.05 * vol_boost
    ...
    def on_bar(self, trade_date: str, bars: Dict[str, Bar], portfolio: PortfolioSnapshot) -> List[Order]:
        ...
        for code, bar in bars.items():
            ...
            vol_ok = avg_vol20 > 0 and vols[-1] >= avg_vol20 * 1.25

            returns = [(closes[i] / closes[i - 1] - 1.0) for i in range(1, len(closes))]
            vol10 = self._stddev(returns[-10:]) if len(returns) >= 10 else 0.0
            vol30 = self._stddev(returns[-30:]) if len(returns) >= 30 else 0.0
            contraction_ok = vol30 > 0 and vol10 <= vol30 * 0.72

            over_extended = bar.close > ema20 * 1.22
            trend_up = ema20 > ema60 > ema120 and ema20_up and bar.close >= ema20 * 0.99
            ...
            pullback_vol_ok = avg_vol20 > 0 and vols[-1] >= avg_vol20 * 0.85
            pullback_ok = (
                trend_up
                and macd_ok
                and 45.0 <= rsi14 <= 62.0
                and ema20 * 0.97 <= bar.close <= ema20 * 1.02
                and pullback_vol_ok
            )
            ...
            if code in self._entry_price:
                ...
                if current_qty > 0 and (stop_hit or trend_exit):
                    orders.append(Order(ts_code=code, action="sell", target_pct=0.0, reason="v9_stop"))
        ...
        desired = [code for code, _ in candidates[:max_positions]] if max_positions > 0 else []
        ...
        for code, qty in portfolio.positions.items():
            ...
            if holding_bars < self.min_hold_bars:
                continue
            orders.append(Order(ts_code=code, action="sell", target_pct=0.0, reason="v9_rotate"))
            self._entry_price.pop(code, None)
            self._peak_price_since_entry.pop(code, None)
            self._entry_index.pop(code, None)
        ...
        for code in desired:
            ...
            orders.append(Order(ts_code=code, action=action, target_pct=target_per_name, reason="v9_rebalance"))
