...
class WatchlistTrendV3Strategy(BaseStrategy):
    ...
    def on_bar(self, trade_date: str, bars: Dict[str, Bar], portfolio: PortfolioSnapshot) -> List[Order]:
        ...
        for code, bar in bars.items():
            ...
            if regime == "Strong" and breakout_ok:
                orders.append(Order(ts_code=code, action="buy", target_pct=1.0, reason="breakout buy"))
                buy_count += 1
            elif regime == "Diverging" and breakout_ok and bar.close >= high20 and rsi14 >= 65.0:
                orders.append(Order(ts_code=code, action="buy", target_pct=1.0, reason="strong breakout buy"))
                buy_count += 1
