...
class WatchlistTrendV8Strategy(BaseStrategy):
    """
    V8 strategy: quality trend + volatility contraction breakout.

    Goals:
    - Improve pool quality via liquidity-aware scoring.
    - Focus on medium/long trend alignment to reduce false signals.
    - Enter on breakouts after volatility contraction; add on controlled pullbacks.
    - Enforce strict stops and minimum holding bars to reduce churn.
    """

    name = "watchlist_trend_v8"
    use_full_bars = True
    top_k = 80

    @classmethod
    def default_score_config(cls) -> ScoreConfig:
        return ScoreConfig(top_n=600, momentum_weight=1.0, volume_weight=0.25, amount_weight=0.15)
    ...
    def __init__(
        ...
        slow_window: int = 60,
        long_window: int = 120,
        hard_stop_loss_pct: float = 0.085,
        ...
        invested_ratio_diverging: float = 0.55,
    ...
    def _score(self, closes: list[float], vols: list[float]) -> float:
        ...
        ret120 = (c0 / closes[-121] - 1.0) if len(closes) >= 121 and closes[-121] > 0 else 0.0
        vol_boost = 0.0
        if len(vols) >= 20:
    ...
    def on_bar(self, trade_date: str, bars: Dict[str, Bar], portfolio: PortfolioSnapshot) -> List[Order]:
        ...
        for code, bar in bars.items():
            ...
            contraction_ok = vol30 > 0 and vol10 <= vol30 * 0.75
            ...
            pullback_vol_ok = avg_vol20 > 0 and vols[-1] >= avg_vol20 * 0.8
            pullback_ok = (
                trend_up
                and macd_ok
                and 45.0 <= rsi14 <= 60.0
            ...
            if code in self._entry_price:
                ...
                if current_qty > 0 and (stop_hit or trend_exit):
                    orders.append(Order(ts_code=code, action="sell", target_pct=0.0, reason="v8_stop"))
        ...
        for code, qty in portfolio.positions.items():
            ...
            if holding_bars < self.min_hold_bars:
                continue
            orders.append(Order(ts_code=code, action="sell", target_pct=0.0, reason="v8_rotate"))
            self._entry_price.pop(code, None)
            self._peak_price_since_entry.pop(code, None)
            self._entry_index.pop(code, None)
        ...
        for code in desired:
            ...
            orders.append(Order(ts_code=code, action=action, target_pct=target_per_name, reason="v8_rebalance"))
