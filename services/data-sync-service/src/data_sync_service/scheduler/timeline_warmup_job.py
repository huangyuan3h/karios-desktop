...
def run() -> dict:
    ...
    try:
        ...
        insert_record(
            ...
            success=True,
            last_ts_code=f"{start}_{today}",
            error_message=f"warmed {len(result.get('rows', []))} rows",
        )
        return result
    except Exception as exc:
        insert_record(
            job_type="timeline_warmup", success=False, last_ts_code="", error_message=str(exc)[:500]
        )
