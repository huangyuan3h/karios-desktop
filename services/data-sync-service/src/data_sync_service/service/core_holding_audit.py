...
Legs (OPT-149): each journal row carries ``leg`` ('s3' | 'parking').
S-3 legs use the backtest-engine rules below (pyramid / stop / panic gate);
non-S-3 legs are outside this rulebook and are not judged here.
...
def _replay_ops(
    ...
    """Judge each S-3 op against its PRE-trade state by reverse-replaying from
    the current (cost, pct) backwards — so the 8/21 ADD is checked against the
    8/20 cost (39.90), not today's blended one.

    Non-S-3 legs (e.g. 'parking') are outside this rulebook and are skipped.
    """
    cur_cost = float(state.get("costPrice") or 0)
    cur_pct = float(state.get("positionPct") or 0)
    judged: list[tuple[int, dict[str, Any]]] = []
    for rev_idx, op in enumerate(reversed(ops)):
        idx = len(ops) - 1 - rev_idx
        if str(op.get("leg") or "s3") != "s3":
            continue
        side = op["side"]
        price = float(op["price"] or 0)
    ...
    for idx, v in judged:
        v.setdefault("id", ops[idx].get("id"))
    return [v for _, v in judged]
...
def audit_core_holdings(*, day: str) -> dict[str, Any]:
    ...
    for row in list_registry():
        ...
        # Holding leg = leg of the most recent BUY (s3 vs non-S-3 never blend rules).
        leg = "s3"
        ...
        holdings_out.append(
            {
                ...
                # Non-S-3 legs (e.g. parking) have no pyramid rule.
                "pyramidTriggerLine": None if leg != "s3" else state.get("pyramidTriggerLine"),
                "pyramidAdded": bool(state.get("pyramidAdded")) and leg == "s3",
                "ops": ops,
            }
        )
