...
def _macro_sentence(m: dict[str, Any], labels: dict[str, str]) -> str:
    ...
    if close is not None:
        ...
        head = f"{label}报{n}" if sid == _SID_USDCNH else f"{label}收报{n}"
    else:
        head = label

    tail: list[str] = []
    if pct is not None:
        tail.append(f"{'涨' if pct >= 0 else '跌'}{abs(pct):.2f}%")
    if as_of:
        tail.append(f"截至{as_of}")
    if tail:
...
def format_market_environment_zh(snapshot: dict[str, Any] | None) -> str:
    ...
    Uses the same shape as GET /macro/snapshot.
    """
    if not snapshot:
        return ""
    parts: list[str] = []
