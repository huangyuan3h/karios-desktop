...
def sync_daily_full() -> dict[str, Any]:
    """
    Full sync: for each stock, sync daily bars from 2024-01-01 to today (or from last date in DB).
    - If today's run already succeeded: skip.
    - If today's run failed: resume from the ts_code after last_ts_code.
    - Upsert by (ts_code, trade_date). If today already has data for a stock, skip that stock.
    """
