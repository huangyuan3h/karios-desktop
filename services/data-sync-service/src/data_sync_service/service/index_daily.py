"""Index daily sync: full sync for selected indices with resume and skip-if-today-ok."""
...
from data_sync_service.db.index_daily import get_last_trade_date, upsert_from_dataframe
...
INDEX_CODES = ["000001.SH", "399006.SZ", "000300.SH"]
INDEX_FIELDS = [
    "ts_code",
    "trade_date",
    "open",
...
def sync_index_daily_full() -> dict[str, Any]:
    """
    Full sync for selected indices:
    - If today's run already succeeded: skip.
    - If today's run failed: resume from the ts_code after last_ts_code.
    - If we already have today's data for an index, skip that index.
    """
    run = get_today_run(JOB_TYPE)
    if run and run.get("success"):
        return {"ok": True, "skipped": True, "message": "already synced today"}

    index_codes = [c.strip().upper() for c in INDEX_CODES if c and c.strip()]
    if not index_codes:
        return {"ok": True, "updated": 0, "message": "no index list"}

    start_index = 0
    if run and run.get("success") is False and run.get("last_ts_code"):
        try:
            idx = index_codes.index(run["last_ts_code"])
            start_index = idx + 1
        except ValueError:
            pass
    ...
    for i in range(start_index, len(index_codes)):
        ...
        try:
            ...
            df: pd.DataFrame = pro.index_daily(
                ts_code=ts_code,
