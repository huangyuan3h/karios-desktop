"""CN income statement (tushare income) — PiT ann_date key.

Source: tushare income (per ts_code, all periods). Stored per
(ts_code, ann_date, end_date, report_type). Hot quant subjects are typed
columns; the full 85-field row is kept in extra JSONB.
"""
...
CREATE INDEX IF NOT EXISTS idx_cn_income_ann_date ON {TABLE_NAME}(ann_date DESC);
CREATE INDEX IF NOT EXISTS idx_cn_income_ts_end ON {TABLE_NAME}(ts_code, end_date DESC);
"""
