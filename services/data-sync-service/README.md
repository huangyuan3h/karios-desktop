...
## Runtime artifacts (OPT-160)

`data/backtest_reports/*_latest.json`, `data/backtest_reports/timeline_cache/`,
`data/twin_star_intraday/*.json` and `data/third_asset_cache.json` are **generated at
runtime / by scripts** and are intentionally **not tracked in git** (see repo
`.gitignore`). They are regenerated on demand; a fresh clone simply starts without
them (the API endpoints that read them return empty until the producer runs).

| Artifact | Regenerated by |
|----------|----------------|
| `walk_forward_latest.json` | `scripts/run_walk_forward.py` |
| `rolling_oos_latest.json` | `scheduler/rolling_oos_job.py` |
| `paper_vs_backtest_latest.json` | `scripts/paper_vs_backtest_report.py` |
| `sleeve_nav_latest.json` | `scripts/sleeve_nav_sim.py` |
| `third_asset_cache.json` | `scripts/sleeve_nav_sim.py` / `run_walk_forward_dual.py` |
| `timeline_cache/*.json` | `api/backtest_routes.py` (disk cache) |
| `twin_star_intraday/*.json` | `scheduler/twin_star_intraday_job.py` |

**Frozen truth stays tracked**: `walk_forward_baseline*.json` (incl. dated baselines
and `walk_forward_hk_baseline*`), `*_frozen_*.json`, `state_sliced_navs/`, and the
`data/index/*.zip` minute archives.
