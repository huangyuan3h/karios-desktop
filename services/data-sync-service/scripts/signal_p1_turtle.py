"""Signal pool · P1 — turtle Donchian breakout (ADDITIVE entry gate).

Hypothesis: requiring close > N-day high on the entry day captures
momentum continuation that RS rank alone misses. ADDITIVE only — it
tightens entries (never replaces RS/score/env).
...
- if adopted: monte-carlo confirm (block bootstrap, like E2)
"""
...
def main() -> int:
    ...
    ap.add_argument("--variants", default="base,N10,N20,N30,N55")
    ...
    json.dump(results, open("data/backtest_reports/signal_p1_turtle.json", "w"), indent=1, ensure_ascii=False)
    print("\nreport -> data/backtest_reports/signal_p1_turtle.json")
    return 0
