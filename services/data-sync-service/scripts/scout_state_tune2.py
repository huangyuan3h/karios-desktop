#!/usr/bin/env python3
"""Per-state tuned Scout (load-once, in-memory loop). Each state uses R1 best valid-ok factor.

Discipline: keep state iff valid daily > +0.100%/day AND three-window all positive.
"""
...
STATE_FACTOR = {
    "S-limit": ("amplitude", "Q1"),
    "S-gap": ("amplitude", "Q1"),
    "S-shrink": ("gap", "Q10"),
    "S-fresh": ("amplitude", "Q1"),
    "S-stress": ("amplitude", "Q1"),
    "S-breakout": ("amplitude", "Q1"),
}
...
def simulate_window(wname, list_dates):
    s, e = WINDOWS[wname]
    ...
    # per-day breadth (cheap) + per-state factor fv computed inside loop
    for day in cal:
        breadth = _day_breadth(per_ts, mv_map, day)
        r_wide = breadth > 0.5
        for st, (factor, q) in STATE_FACTOR.items():
            ...
            if r_wide and day in date_to_idx and date_to_idx[day] > 0:
                sig_fv, _ = _day_state_fv_factor(per_ts, mv_map, list_dates, cal[date_to_idx[day] - 1], factor)
                fmap = sig_fv.get(st, {})
...
def _day_breadth(per_ts, mv_map, day):
    above = 0; tot = 0
    for ts, series in per_ts.items():
        idx = -1
        for i, r in enumerate(series):
        ...
        if series[idx]["close"] > sum(closes) / 20:
            above += 1
    return (above / tot) if tot else 0.0


def _day_state_fv_factor(per_ts, mv_map, list_dates, day, factor):
    day_fv = {}; day_all = {}
    for ts, series in per_ts.items():
        idx = -1
        for i, r in enumerate(series):
...
def main():
    ...
    for w in WINDOWS:
        ...
        all_res[w] = simulate_window(w, list_dates)
    ...
    for st, (factor, q) in STATE_FACTOR.items():
        ...
        keep = (o["daily"] > 0 and t["daily"] > 0 and v["daily"] > 0 and v["daily"] > 0.100)
        (keeps if keep else rejects).append(st)
        print(f"| {st:10s} | {factor}/{q} | {o['daily']:+.4f} | {t['daily']:+.4f} | {v['daily']:+.4f} | {v['max_dd']:.1f} | {v['sharpe']:.2f} | {'✅' if keep else '❌'} |")
    out = REPORT_DIR / "state_tune_latest.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps({"state_factor": STATE_FACTOR, "baseline_daily": 0.100, "keeps": keeps, "rejects": rejects, "results": all_res}, ensure_ascii=False, indent=2, default=str))
    print(f"KEEP: {keeps}")
