"""Signal pool · P2 — volume breakout (ADDITIVE entry gate).

Hypothesis: entry-day volume above K x 20-day average confirms real
buying (放量突破), filtering RS/score-worthy names that move on thin tape.
Volume is invisible to RS/score/env → a true additive dimension.

Variants (mult K on the 20d average volume; some stacked with P1 N20
breakout to test the 放量突破 combo):
- base   (off)
- V1.5   volume > 1.5x 20d avg
- V2.0   volume > 2.0x 20d avg
- V1.5B  volume > 1.5x 20d avg AND close > 20d high (放量突破 combo)
- V2.0B  volume > 2.0x 20d avg AND close > 20d high
...
def main() -> int:
    ...
    ap.add_argument("--variants", default="base,V1.5,V2.0,V1.5B,V2.0B")
    ...
    json.dump(results, open("data/backtest_reports/signal_p2_volume.json", "w"), indent=1, ensure_ascii=False)
    print("\nreport -> data/backtest_reports/signal_p2_volume.json")
    return 0
