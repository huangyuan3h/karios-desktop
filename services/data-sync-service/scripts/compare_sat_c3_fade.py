#!/usr/bin/env python3
"""S2: habit satellite C3 fade filter — full-day amp vs no-lookahead keys.

Base is the current habit Live (C1 3% upside cap, same_1430 fill, body=3,
day-3 1430 exit, rank amp, bucket top-1/3, R-wide 0.5). Variants add ONLY
the C3 downside filter (skip when 14:30/open-1 < -X): exclude names that
faded into the fill. Loss-exclusion direction (S2), not number tuning.
Selection on OOS2+train; valid only validates.
...
VARIANTS: tuple[dict[str, object], ...] = (
    {"id": "c1", "label": "C1 3% (Live, no fade filter)", "c3_pct": None},
    {"id": "c1c3_03", "label": "C1 3% + C3 skip runup<-3%", "c3_pct": 0.03},
    {"id": "c1c3_02", "label": "C1 3% + C3 skip runup<-2%", "c3_pct": 0.02},
)
...
def main() -> int:
    ...
    print("S2 habit C3 fade filter: none vs -2% vs -3%\n")
    ...
    for wname, (s, e) in WINDOWS.items():
        ...
        for var in VARIANTS:
            c3_pct = var["c3_pct"]
            assert c3_pct is None or isinstance(c3_pct, float)
    ...
    if args.save_report:
        ...
        path = REPORT_DIR / "sat_c3_fade_2026-09-04.json"
