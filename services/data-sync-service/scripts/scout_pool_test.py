...
WINDOWS = {"valid": ("2026-03-01","2026-08-07"), "long": ("2021-08-01","2026-08-07"), "OOS2": ("2024-08-01","2025-08-01")}

def run(min_mv, max_mv, wname):
    ...
    # load mv
    s2 = max(date.fromisoformat(start)-timedelta(days=5), date(1998,1,1)).isoformat()
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT trade_date, ts_code, total_mv FROM stock_dailybasic WHERE trade_date >= %s AND trade_date <= %s AND total_mv IS NOT NULL", (s2,end))
            rows=cur.fetchall()
    mv_map={}
    for d,ts,mv in rows:
    ...
    # simulate amp_and_gap 10d breadth>0.5
            ...
            if idx>0:
                ...
                if breadth.get(sig_day,0) < 0.5:
                    ...
                    if gap:
                        ...
                        gap_q5=set(ts for ts in gap_q5 if gap[ts]>0.01)
    ...
    avg_pos=sum(len([p for p in positions]) for _ in [1]) # placeholder
    # approximate hold ratio from trades: total position days / (n_days*10)
    # we have pos_counts not tracked, approximate via trades*hold/n_days/10
    hold_ratio = len(trades)*10 / n_days /10 *100 if n_days else 0 # rough
    return {"pool":f"{min_mv}-{max_mv}","window":wname,"total_pnl":total_pnl,"daily_avg":daily_avg,"max_dd":max_dd,"win_rate":win_rate,"trades":len(trades)}

pools=[(20,80),(20,150),(20,300),(50,300),(0,300)]
for min_mv,max_mv in pools:
    for w in ["valid","long","OOS2"]:
        res=run(min_mv,max_mv,w)
        print(f"pool {res['pool']:7s} {w:6s} total {res['total_pnl']:+6.1f}% daily {res['daily_avg']:+.4f}% dd {res['max_dd']:4.1f} win {res['win_rate']*100:4.1f}% n {res['trades']}")
