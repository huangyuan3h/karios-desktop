#!/usr/bin/env python3
"""H-SAT-RANK full scorecard: amp vs stage re-rank (twin tot / Sharpe / maxDD).

Base is the current habit Live (C1 3% same_1430 fill, body=3, day-3 1430
exit, rank by full-day amplitude). Variants change ONLY the S-gap ranking:
  rank_stage       stage order incl. decision-day close (2026-09-11 PASS def)
  rank_stage_prev  stage order through prior session (no same-day lookahead)

Score twin tot+Sharpe+maxDD vs core (beats_core) and vs base (±5pt REJECT),
across OOS2 / train / valid + past_year / aligned. Frozen engine untouched
for Live (rank_key is experiment-only).

Usage:
  cd services/data-sync-service
  PYTHONPATH=src:scripts python3 scripts/compare_sat_rank_stage.py --save-report
"""
...
VARIANTS: tuple[dict[str, object], ...] = (
    {"id": "rank_amp", "label": "C1 3% · rank full-day amp (Live)", "rank_key": None},
    {"id": "rank_stage", "label": "C1 3% · stage re-rank (incl day close)", "rank_key": "stage"},
    {"id": "rank_stage_1430", "label": "C1 3% · stage re-rank (14:30 print = Live)", "rank_key": "stage_1430"},
    {"id": "rank_stage_prev", "label": "C1 3% · stage re-rank (no lookahead)", "rank_key": "stage_prev"},
)
...
def main() -> int:
    ...
    print("H-SAT-RANK full scorecard: amp vs stage re-rank\n")
    ...
    for wname, (s, e) in WINDOWS.items():
        ...
        for var in VARIANTS:
            ...
            assert rank_key is None or isinstance(rank_key, str)
    ...
    payload = {
        "tag": "sat-rank-stage-2026-09-11",
        "protocol": (
            "window-local empty book; clip4 C1 3% same_1430 fill; body=3; "
            "day-3 1430 exit; frozen pick-strong trail8; opp_50. "
            "Variants differ ONLY in S-gap rank key. "
            "Score tot+Sharpe+maxDD vs core and vs base."
        ),
        "variants": VARIANTS,
        "windows": results,
        "verdicts": verdicts,
    ...
    if args.save_report:
        ...
        path = REPORT_DIR / "sat_rank_stage_2026-09-11.json"
