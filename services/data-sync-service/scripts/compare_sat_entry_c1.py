#!/usr/bin/env python3
"""S-gap 14:30 habit calendar: C1/C2 entry filters vs unfiltered same_1430.

Habit: today's S-gap, fill at bar_5min 14:30, body=3 close. Unfiltered
same_1430 already loses to core on train/valid (see sat-fill-same-close).

C1: skip if 14:30 / today's open - 1 is too large (pulse already spent).
C2: skip if 14:30 is already hugging the board limit.
Strict clip4: skipped names do not refill from worse ranks; idle → core.

This is NOT a rewrite of frozen T-open. Scoreboard (total + Sharpe + maxDD, all three windows):
  vs same_1430 twin: total >5pt worse on any OOS2/train/valid → REJECT the filter.
    Sharpe down or maxDD up vs same_1430 is a risk fail even if total passes.
  vs core (habit product): all three windows must beat core on total AND
    Sharpe >= core AND maxDD <= core. Tot-only wins do not count as Live.

Live does not change in this script.

Usage:
  cd services/data-sync-service
  PYTHONPATH=src:scripts python3 scripts/compare_sat_entry_c1.py
...
VARIANTS: tuple[dict[str, object], ...] = (
    {
        "id": "same_1430",
        "label": "today S-gap → 14:30 · no extra filter",
        "max_open_to_1430_pct": None,
        "near_limit_buffer_pct": None,
    },
    {
        "id": "c1_run03",
        "label": "C1 skip if 14:30/open − 1 > 3%",
        "max_open_to_1430_pct": 0.03,
        "near_limit_buffer_pct": None,
    },
    {
        "id": "c1_run05",
        "label": "C1 skip if 14:30/open − 1 > 5%",
        "max_open_to_1430_pct": 0.05,
        "near_limit_buffer_pct": None,
    },
    {
        "id": "c2_near01",
        "label": "C2 skip if 14:30 within 1% of board limit",
        "max_open_to_1430_pct": None,
        "near_limit_buffer_pct": 0.01,
    },
    {
        "id": "c1_run03_c2",
        "label": "C1 3% + C2 near-limit",
        "max_open_to_1430_pct": 0.03,
        "near_limit_buffer_pct": 0.01,
    },
)
...
def _habit_tag(tot: list[float], sr: list[float], dd: list[float]) -> str:
    """Live-habit scoreboard vs core: total, Sharpe, and maxDD all required."""
    flags: list[str] = []
...
def main() -> int:
    ...
    print("Sat 14:30 entry filter C1/C2 vs unfiltered same_1430\n")
    ...
    for wname, (s, e) in WINDOWS.items():
        ...
        for var in VARIANTS:
            sat = replay_sgap_from_context(
                ...
                max_open_to_1430_pct=var["max_open_to_1430_pct"],  # type: ignore[arg-type]
                near_limit_buffer_pct=var["near_limit_buffer_pct"],  # type: ignore[arg-type]
            )
            ...
            row[str(var["id"])] = {
                "label": var["label"],
                "max_open_to_1430_pct": var["max_open_to_1430_pct"],
                "near_limit_buffer_pct": var["near_limit_buffer_pct"],
                "sat": sat_m,
                "twin": twin_m,
                ...
                "skipT1Count": summary.get("skipT1Count"),
                "skipC1Count": summary.get("skipC1Count"),
                "skipC2Count": summary.get("skipC2Count"),
                "avgHeldDays": summary.get("avgHeldDays"),
                "closeReasons": summary.get("closeReasons") or {},
                **occ,
            }
            print(
                f"  {var['id']:<14} twin {_fmt(twin_m)}  "
                f"Δcore tot {delta_core:+.1f} sr {delta_core_sr:+.2f} dd {delta_core_dd:+.1f}  "
                f"sat {_fmt(sat_m)}  pos {occ['avg_pos_active']:.1f} "
                f"(act {occ['pct_active']:.0f}%)  fills {summary.get('fillCount')}  "
                f"skipT1 {summary.get('skipT1Count')} C1 {summary.get('skipC1Count')} "
                f"C2 {summary.get('skipC2Count')}",
                flush=True,
            )
        assert core_m is not None
        base = row["same_1430"]["twin"]
    ...
    print("\n## Twin NAV (total/Sharpe/maxDD) vs unfiltered same_1430\n")
    ...
    for wname in WINDOWS:
        ...
        for v in VARIANTS:
            ...
            extra = f" ({rec['delta_base_pt']:+.1f})" if v["id"] != "same_1430" else ""
            cells.append(f"{_fmt(rec['twin'])}{extra}")
        print(f"| {wname} | " + " | ".join(cells) + " |")

    print("\n## Twin Sharpe vs core (Δ; negative = worse)\n")
    print("| 窗口 | 核心 sr | " + " | ".join(v["id"] for v in VARIANTS) + " |")
    print("|" + "|".join(["------"] * (2 + len(VARIANTS))) + "|")
    for wname in WINDOWS:
        core_sr = results[wname]["core"]["sharpe"]
        cells = [f"{core_sr:.2f}"]
        for v in VARIANTS:
            rec = results[wname]["variants"][str(v["id"])]
            cells.append(f"{rec['twin']['sharpe']:.2f} ({rec['delta_core_sharpe']:+.2f})")
        print(f"| {wname} | " + " | ".join(cells) + " |")

    print("\n## Twin maxDD vs core (Δ; positive = worse drawdown)\n")
    print("| 窗口 | 核心 DD | " + " | ".join(v["id"] for v in VARIANTS) + " |")
    print("|" + "|".join(["------"] * (2 + len(VARIANTS))) + "|")
    for wname in WINDOWS:
        core_dd = results[wname]["core"]["max_dd"]
        cells = [f"{core_dd:.1f}"]
        for v in VARIANTS:
            rec = results[wname]["variants"][str(v["id"])]
            cells.append(f"{rec['twin']['max_dd']:.1f} ({rec['delta_core_dd']:+.1f})")
        print(f"| {wname} | " + " | ".join(cells) + " |")

    print("\n## Walk-forward: total / Sharpe / maxDD (dd Δ>0 = worse drawdown)\n")
    ...
    for v in VARIANTS:
        ...
        if vid == "same_1430":
            ...
            print(f"- {vid} (baseline): vs core " + _habit_line(vs_core_tot, vs_core_sr, vs_core_dd) + f" → {harvest}")
            continue
    ...
    payload = {
        "tag": "sat-entry-c1-2026-09-03",
        "protocol": (
            "window-local empty book; frozen clip4 sat (4×25% strict skip_t1); "
            "fill_mode=same_1430 (today gap, 14:30 print); C1 max_open_to_1430 / "
            "C2 near_limit_buffer; idle slots → core; frozen pick-strong trail8; "
            "opp_50. Habit scoreboard is twin vs core on total+Sharpe+maxDD. "
    ...
    if args.save_report:
        ...
        path = REPORT_DIR / "sat_entry_c1_2026-09-03.json"
