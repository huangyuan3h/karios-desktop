#!/usr/bin/env python3
"""H-SAT-W: Harbor x habit-satellite exposure curve (pre-registered 2026-09-14).

Frozen caliber (no tuning): core = Harbor P1 (single-source parking_replay),
satellite = live habit S-gap replay; blend = blend_nav_opportunity.
Grid: w in {0.10, 0.15, 0.20, 0.25, 1/3, 0.40, 0.50}; windows OOS2/train/valid/long
(+holdout descriptive). Verdict: K1 worst-window dTotal >= -5pt; K2 long Sharpe
>= core+0.10 and long dTotal >= +25pt; K3 long MDD not worse and valid MDD not
worse by >2pt; pick the largest w passing all three.
...
def main() -> int:
    ...
    for w in (*WINS, "holdout"):
        ...
        corr_act = _corr([core_rets[i] for i in act_idx], [sat_rets[i] for i in act_idx])
        grid: dict[str, dict] = {}
        for gw in GRID:
            ...
            grid[str(round(gw, 4))] = {
                **m,
                "delta_total": round(m["total_pct"] - core_m["total_pct"], 1),
                "delta_sharpe": round(m["sharpe"] - core_m["sharpe"], 2),
                "delta_mdd": round(m["max_dd"] - core_m["max_dd"], 1),
            }
        results[w] = {
            "core": core_m, "satellite": sat_m, "grid": grid,
            "pct_active": round(100 * sum(1 for a in active if a) / max(1, n), 1),
            "fills": sat["summary"].get("fillCount"),
            "corr_all": corr_all, "corr_active": corr_act, "n_days": n,
        }
        print(f"  core {_fmt(core_m)} | sat {_fmt(sat_m)} | active {results[w]['pct_active']}% "
              f"corr all/act {corr_all}/{corr_act}", flush=True)
    ...
    # verdict (frozen): K1/K2/K3 + largest passing w
    ...
    print(f"  chosen: {chosen if chosen is not None else 'NONE -> REJECT (keep Live=Harbor)'}")
    hv = results.get("holdout")
    if hv:
        print(f"  [descriptive] holdout core {_fmt(hv['core'])} | sat {_fmt(hv['satellite'])} | "
              f"w=0.25 {_fmt(hv['grid']['0.25'])}")

    if args.save_report:
        REPORT_DIR.mkdir(parents=True, exist_ok=True)
        (REPORT_DIR / "harbor_sat_weight_2026-09-14.json").write_text(
            json.dumps(
                {
                    "tag": "h-sat-w-2026-09-14",
                    "prereg": "docs/designs/harbor-sat-weight-prereg-2026-09-14.md",
