#!/usr/bin/env python3
"""H1: habit satellite ranking — full-day amp vs no-lookahead keys.
...
exit, rank by full-day amplitude). Variants change ONLY the S-gap ranking:
  rank_gap    gap% ascending (known at open, zero missing data)
  rank_absrun |1430/open-1| ascending (known at fill; missing print ranks last)

Diagnostic (OOS2+train terciles, scripts/diag_sat_rank_keys.py): amp spread
+1.67, gap +1.27, |runup| +0.80, drift ~0 (dropped), signed runup reversed
(dropped: counterintuitive, bounce-risk).

Score twin tot+Sharpe+maxDD vs core (beats_core) and vs base (±5pt REJECT).
Frozen engine untouched (rank_key is experiment-only). Do not rewrite Live.
...
VARIANTS: tuple[dict[str, object], ...] = (
    ...
    {"id": "rank_gap", "label": "C1 3% · rank gap% asc", "rank_key": "gap_asc"},
    {"id": "rank_absrun", "label": "C1 3% · rank |1430/open-1| asc", "rank_key": "absrunup_asc"},
)
...
def main() -> int:
    ...
    print("H1 habit rank: amp vs gap_asc vs absrunup_asc\n")
    ...
    if args.save_report:
        ...
        path = REPORT_DIR / "sat_rank_hhmm_2026-09-04.json"
