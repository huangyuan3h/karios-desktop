"""Signal pool · P14 — PEAD (post-earnings drift) gate.

Hypothesis: names that just announced a POSITIVE earnings surprise
(业绩预告 预增/扭亏/略增/续盈) drift upward as information diffuses
slowly (PEAD is a well-documented A-share anomaly). Restricting entries
to names with a positive forecast within the last N sessions tests
whether S-3's momentum entries benefit from an earnings confirmation.

Variants (N = sessions after a positive forecast within which entry is
allowed):
- base   (off)
- N10    entry only within 10 sessions of a positive forecast
- N20    entry only within 20 sessions
- N30    entry only within 30 sessions
- N60    entry only within 60 sessions
...
def summarize(run: be.BacktestRun) -> dict:
    ...
    return {
        ...
        "pead": int((s.gated_blocks or {}).get("pead", 0)),
    }
...
def main() -> int:
    ...
    ap.add_argument("--variants", default="base,N10,N20,N30,N60")
    ...
    for name in windows:
        ...
        for v in variants:
            cfg = be.BacktestConfig(start_date=s, end_date=e, **S3_CONFIG, pead_days=VARIANTS[v])
            run = be.simulate(cfg)
            m = summarize(run)
            results[name][v] = m
            print(
                f"  {v:8s} {m['pnl']:+8.1f} {m['dd']:6.1f} {m['calmar']:7.2f} "
                f"{m['sharpe']:7.2f} {m['win']*100:5.1f}% {m['closed']:6d} {m['pead']:9d}"
            )
    ...
    json.dump(results, open("data/backtest_reports/signal_p14_pead.json", "w"), indent=1, ensure_ascii=False)
    print("\nreport -> data/backtest_reports/signal_p14_pead.json")
    return 0
