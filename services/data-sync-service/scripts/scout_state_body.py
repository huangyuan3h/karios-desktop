#!/usr/bin/env python3
"""R5 eat-the-body scheme: per-state optimal hold (S-limit/S-gap=3d, S-fresh/S-shrink=15d), union, R-wide gate.

Body lengths from state_hold_grid: hot states body 3d, slow states body 15d.
Compare vs R3 uniform-h10 (+0.330%/day).
"""
...
def simulate_window(wname, list_dates):
    ...
    for ts, series in per_ts.items():
        ...
        if m:
            close_by_ts[ts] = m
    nav_state = {st: [] for st in STATE_HOLD}
    realized = {st: 0.0 for st in STATE_HOLD}
    positions = {st: {} for st in STATE_HOLD}
    for day in cal:
        state_fv, breadth = _day_state_fv(per_ts, mv_map, list_dates, cal, day)
        r_wide = breadth > 0.5
        for st, hold in STATE_HOLD.items():
            pos = positions[st]
            to_close = []
...
def main():
    ...
    print("\n### Eat-the-body scheme (per-state hold) ###")
    print("| state | hold | OOS2 daily | train daily | valid daily | valid dd | sharpe |")
    print("|-------|------|-----------|-----------|-----------|---------|--------|")
    for st, hold in STATE_HOLD.items():
        o = all_res["OOS2"][st]; t = all_res["train"][st]; v = all_res["valid"][st]
        print(f"| {st:10s} | {hold:2d} | {o['daily']:+.4f} | {t['daily']:+.4f} | {v['daily']:+.4f} | {v['max_dd']:.1f} | {v['sharpe']:.2f} |")
    out = REPORT_DIR / "state_body_latest.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps({"state_hold": STATE_HOLD, "baseline_uniform_h10_daily": 0.330, "results": all_res}, ensure_ascii=False, indent=2, default=str))
    print("R3 uniform-h10 valid: +0.330/day. Eat-body scheme vs above.")
    print(f"report -> {out}")
