#!/usr/bin/env python3
"""H3: habit satellite bucket_q — full-day amp vs no-lookahead keys.

Base is the current habit Live (C1 3% same_1430 fill, body=3, day-3 1430
exit, rank by full-day amplitude, bucket top-1/3). Variants change ONLY
bucket_q (top-1/2 vs top-1/3 feeding strict 4 slots). 4 slots fixed
(followability constraint). Selection on OOS2+train; valid only validates.
...
VARIANTS: tuple[dict[str, object], ...] = (
    {"id": "bq2", "label": "bucket top-1/2 · wider", "bucket_q": 2},
    {"id": "bq3", "label": "bucket top-1/3 (Live)", "bucket_q": 3},
)
...
def main() -> int:
    ...
    print("H3 habit bucket_q: top-1/2 vs top-1/3\n")
    ...
    for wname, (s, e) in WINDOWS.items():
        ...
        row: dict[str, dict] = {}
        for var in VARIANTS:
            bucket_q = var["bucket_q"]
            assert isinstance(bucket_q, int)
    ...
    if args.save_report:
        ...
        path = REPORT_DIR / "sat_bucketq_2026-09-04.json"
