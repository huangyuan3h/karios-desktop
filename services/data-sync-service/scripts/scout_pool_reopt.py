...
WINDOWS = {"valid": ("2026-03-01","2026-08-07"), "OOS2": ("2024-08-01","2025-08-01"), "long": ("2021-08-01","2026-08-07")}

def run_one(min_mv, max_mv, scheme, wname):
...
for min_mv,max_mv in pools:
    for scheme in schemes:
        for w in ["valid","OOS2","long"]:
            res=run_one(min_mv,max_mv,scheme,w)
            print(f"pool {res['pool']:7s} {scheme:2s} {w:6s} total {res['total_pnl']:+6.1f}% daily {res['daily_avg']:+.4f}% dd {res['max_dd']:4.1f} win {res['win_rate']*100:4.1f}% n {res['trades']}")
