"""Signal pool · P12 — volatility-adjusted momentum (ADDITIVE entry gate).

Hypothesis: requiring ret_120 / stdev(daily returns over 60 sessions) >= X
filters for names whose trend is strong PER UNIT OF RISK. Expected to cut
high-vol chase entries (妖股反噬) and improve sharpe / max-DD / Calmar
rather than raw pnl. RS rank is about WHERE in the cross-section; this is
about HOW risky the trend is — orthogonal-ish dimension.

Variants (ret_120 / vol_60 threshold):
- base   (off)
- V0.5   ret/vol >= 0.5
- V1.0   ret/vol >= 1.0
- V1.5   ret/vol >= 1.5
...
- for a risk-quality filter: sharpe/Calmar/DD improvement counts even at
  flat pnl, but pnl may not degrade >5pt in any window
"""
...
VARIANTS = {
    "base": None,  # ret_days=0 -> gate fully off
    "V0.5": 0.5,
    "V1.0": 1.0,
    "V1.5": 1.5,
    "V2.0": 2.0,
...
def summarize(run: be.BacktestRun) -> dict:
    ...
    return {
        ...
        "ram": int((s.gated_blocks or {}).get("risk_adj_mom", 0)),
    }
...
def main() -> int:
    ...
    ap.add_argument("--variants", default="base,V0.5,V1.0,V1.5,V2.0")
    ...
    for name in windows:
        ...
        results[name] = {}
        print(f"\n=== {name} ({s}..{e}) ===")
        print(f"  {'variant':8s} {'pnl%':>8s} {'dd%':>6s} {'calmar':>7s} {'sharpe':>7s} {'win':>6s} {'closed':>6s} {'ramBlock':>8s}")
        for v in variants:
            th = VARIANTS[v]
            cfg = be.BacktestConfig(
                start_date=s, end_date=e, **S3_CONFIG,
                risk_adj_mom_ret_days=(0 if th is None else 120),
                risk_adj_mom_vol_days=60,
                risk_adj_mom_min=(th or 0.0),
            )
            run = be.simulate(cfg)
            m = summarize(run)
            results[name][v] = m
            print(
                ...
                f"{m['sharpe']:7.2f} {m['win']*100:5.1f}% {m['closed']:6d} {m['ram']:8d}"
            )
    ...
    json.dump(results, open("data/backtest_reports/signal_p12_risk_adj_mom.json", "w"), indent=1, ensure_ascii=False)
    print("\nreport -> data/backtest_reports/signal_p12_risk_adj_mom.json")
    return 0
