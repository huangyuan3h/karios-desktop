#!/usr/bin/env python3
"""H2: habit satellite C1 grid — full-day amp vs no-lookahead keys.

Base is the current habit Live (C1 3% same_1430 fill, body=3, day-3 1430
exit, rank by full-day amplitude). Variants change ONLY the C1 threshold
(skip when 14:30/open-1 > X). Selection on OOS2+train; valid only validates.
...
VARIANTS: tuple[dict[str, object], ...] = (
    {"id": "c1_02", "label": "C1 2% · strict", "c1_pct": 0.02},
    {"id": "c1_03", "label": "C1 3% (Live)", "c1_pct": 0.03},
    {"id": "c1_04", "label": "C1 4% · loose", "c1_pct": 0.04},
    {"id": "c1_05", "label": "C1 5% · loosest", "c1_pct": 0.05},
)
...
def main() -> int:
    ...
    print("H2 habit C1 grid: 2% vs 3% vs 4% vs 5%\n")
    ...
    for wname, (s, e) in WINDOWS.items():
        ...
        for var in VARIANTS:
            c1_pct = var["c1_pct"]
            assert isinstance(c1_pct, float)
    ...
    payload = {
        "tag": "sat-c1-grid-2026-09-04",
        "protocol": (
            "window-local empty book; clip4 same_1430 fill; body=3; "
            "day-3 1430 exit; frozen pick-strong trail8; opp_50. "
            "Variants differ ONLY in C1 threshold (skip 1430/open-1 > X). "
    ...
    if args.save_report:
        ...
        path = REPORT_DIR / "sat_c1_grid_2026-09-04.json"
