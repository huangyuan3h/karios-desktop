#!/usr/bin/env python3
"""H4: habit satellite R-wide gate — full-day amp vs no-lookahead keys.

Base is the current habit Live (C1 3% same_1430 fill, body=3, day-3 1430
exit, rank by full-day amplitude, bucket top-1/3, R-wide breadth>0.5).
Variants change ONLY the R-wide gate (0.4/0.5/0.6). Lowest priority:
the gate is the easiest knob to overfit. Selection on OOS2+train;
valid only validates. A split selection verdict means KEEP 0.5.
...
VARIANTS: tuple[dict[str, object], ...] = (
    {"id": "rw04", "label": "R-wide >0.4 · looser gate", "r_wide": 0.4},
    {"id": "rw05", "label": "R-wide >0.5 (Live)", "r_wide": 0.5},
    {"id": "rw06", "label": "R-wide >0.6 · tighter gate", "r_wide": 0.6},
)
...
def main() -> int:
    ...
    print("H4 habit R-wide gate: 0.4 vs 0.5 vs 0.6\n")
    ...
    for wname, (s, e) in WINDOWS.items():
        ...
        for var in VARIANTS:
            r_wide = var["r_wide"]
            assert isinstance(r_wide, float)
            sat = replay_sgap_from_context(
                ...
                r_wide=None if r_wide == 0.5 else r_wide,
            )
    ...
    if args.save_report:
        ...
        path = REPORT_DIR / "sat_rwide_2026-09-04.json"
