"""G1 diagnostic (P0-11): ccr, size-neutral — pre-registered, single hypothesis.

Hypothesis H-G1: within size quintiles, high cash-conversion (CFO-TTM /
NI-TTM) stocks have higher 60d forward returns (F2's weak raw signal was
...
PiT: knowable at max(income, cashflow ann_date); universe non-financials,
NI-TTM > 0 (same as F2).

Usage:
    PYTHONPATH=src python3 scripts/diag_fin_g1.py
"""
...
def main() -> int:
    ...
    panel = panel[~panel["is_fin"] & panel["ccr"].notna()].copy()
    ...
    for end, g in panel.groupby("end_date"):
        ...
        for _, b in g.groupby("size_q"):
            if len(b) < 50:
                continue
            ric = _spearman(b["ccrw"].tolist(), b["fwd60"].tolist())
            b = b.copy()
            b["q"] = pd.qcut(b["ccrw"], 5, labels=False, duplicates="drop")
            qm = b.groupby("q")["fwd60"].mean()
            sp = (qm.iloc[-1] - qm.iloc[0]) if len(qm) == 5 else None
            mc = _spearman(b["ccrw"].rank().tolist(), b["mom60"].rank().tolist())
            if ric is not None:
                ics.append(ric)
            if sp is not None:
                spreads.append(sp)
        ...
        if not ics or not spreads:
            continue
        sc = _spearman(g["ccrw"].rank().tolist(), g["mv"].rank().tolist())
        rows.append({
