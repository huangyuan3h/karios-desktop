#!/usr/bin/env python3
"""Backfill seat-level dragon-tiger list (龙虎榜营业部明细) 2021+ via tushare top_inst.

Per trading day, all stocks' disclosed buy/sell seats with amounts and reasons.
This is the seat-level twin of sync_lhb_inst.py (which stores 机构 aggregates only).

Output: data/lhb/lhb_seats.csv (resume-safe: dates already present are skipped).

Usage:
  cd services/data-sync-service
  PYTHONPATH=src python3 scripts/sync_lhb_seats.py --limit 5          # smoke
  PYTHONPATH=src python3 scripts/sync_lhb_seats.py --start 2021-01-01
  PYTHONPATH=src python3 scripts/sync_lhb_seats.py --sleep 0.8        # slow down on quota
"""
...
OUT = Path(__file__).resolve().parents[1] / "data" / "lhb" / "lhb_seats.csv"
...
def _fetch_day(pro: Any, trade_date: str, *, retries: int = 3) -> list[dict[str, Any]]:
    ...
    for attempt in range(retries):
        try:
            df = pro.top_inst(trade_date=compact)
            ...
            time.sleep(wait)
    raise RuntimeError(f"top_inst {compact} failed: {str(last_err)[:200]}")
...
def main() -> int:
    ...
    logger.info(
        "days=%d done=%d pending=%d source=top_inst out=%s",
        len(days),
        len(done),
        len(pending),
        OUT,
