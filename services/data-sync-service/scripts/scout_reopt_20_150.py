...
UNIVERSE_MIN_MV, UNIVERSE_MAX_MV = 20, 150
POSITION_PCT, MAX_POSITIONS, COSTS_ROUNDTRIP = 0.10, 10, 0.003
...
def _load_daily(s,e):
    ...
    return per_ts
def _breadth(per_ts,mv_map,cal):
    breadth={}
    for day in cal:
        cnt=tot=0
...
def simulate(wname, pattern, hold):
                ...
                if breadth.get(sig_day,0) < 0.5:
                    ...
                    if pattern=="amp_q10":
                        ...
                        if amp and turn:
                            ...
                            qa=max(1,len(sa)*10//100); qt=max(1,len(st)*10//100)
                            a_q=set(ts for ts,_ in sa[:qa]); t_q=set(ts for ts,_ in st[:qt])
                            cands=a_q & t_q
                    elif pattern=="amp_gap":
                        if amp and gap:
                            sa=sorted(amp.items(),key=lambda kv:kv[1])
    ...
    return {"pattern":pattern,"hold":hold,"window":wname,"total_pnl":total_pnl,"daily_avg":daily_avg,"max_dd":max_dd,"win_rate":win_rate,"trades":len(trades)}

if __name__ == "__main__":
    patterns=["amp_q10","amp_q5","amp_turn_q10","amp_gap"]
    holds=[5,10,15]
    for pat in patterns:
        for hold in holds:
            for w in WINDOWS:
                res=simulate(w,pat,hold)
                print(f"{pat:13s} h{hold:2d} {w:6s} daily {res['daily_avg']:+.4f}% total {res['total_pnl']:+6.1f}% dd {res['max_dd']:4.1f} win {res['win_rate']*100:4.1f}% n {res['trades']}")
    ...
    for pat in patterns:
        for hold in holds:
            for w in WINDOWS:
                all_res.append(simulate(w,pat,hold))
    grouped=defaultdict(dict)
    for r in all_res:
        grouped[(r["pattern"],r["hold"])][r["window"]]=r
    print("\n=== Rank 20-150 pool by valid daily ===")
    ranked=[]
    ...
    for v_daily, tri_avg, key, d in ranked:
        ...
        print(f"{key} valid {v_daily:+.4f}% tri {tri_avg:+.4f}% | OOS2 {o['total_pnl']:+5.1f}/{o['daily_avg']:+.4f} train {t['total_pnl']:+5.1f}/{t['daily_avg']:+.4f} valid {v['total_pnl']:+5.1f}/{v['daily_avg']:+.4f}")
