"""Manual one-shot sync for HK daily K-lines via yfinance.
...
def main(argv: list[str] | None = None) -> int:
    ...
    for i, tc in enumerate(ts_codes, 1):
        ...
        if ok:
            ...
            if updated > 0:
                succeeded += 1
                print(f"[{i}/{len(ts_codes)}] {tc}: ok, +{updated} bars")
            else:
                skipped += 1
                print(f"[{i}/{len(ts_codes)}] {tc}: skipped (no new bars from yfinance)")
        else:
            failed += 1
