#!/usr/bin/env python3
"""R4 per-state hold grid: find each kept state's 'body' length (eat body not tail).

Kept states: S-limit, S-gap, S-fresh, S-shrink. Factor: amplitude Q1 (R1 best valid-ok).
R-wide gate. hold grid {3,5,10,15}. Report valid daily per (state, hold).
Goal: each state's optimal hold = its body length; shorter holds cut tail, longer hold extends into tail.
"""
...
STATE_FACTOR = {"S-limit": "amplitude", "S-gap": "amplitude", "S-fresh": "amplitude", "S-shrink": "amplitude"}
HOLDS = [15]
...
def _day_state_fv(per_ts, mv_map, list_dates, cal, day, factor):
    day_fv = {}; day_all = {}
    for ts, series in per_ts.items():
        ...
        if fresh:
            states.add("S-fresh")
        fv = (cur["high"] - cur["low"]) / cur["close"] if factor == "amplitude" else np.nan
        day_fv[ts] = fv
...
def simulate_window(wname, list_dates, hold):
    s, e = WINDOWS[wname]
    ...
    for ts, series in per_ts.items():
        ...
        if m:
            close_by_ts[ts] = m
    nav_state = {st: [] for st in STATE_FACTOR}
    realized = {st: 0.0 for st in STATE_FACTOR}
    positions = {st: {} for st in STATE_FACTOR}
    for day in cal:
        state_fv, breadth = _day_state_fv(per_ts, mv_map, list_dates, cal, day, "amplitude")
        r_wide = breadth > 0.5
        for st in STATE_FACTOR:
            pos = positions[st]
            to_close = []
            for ts, p in list(pos.items()):
            ...
            for ts in to_close:
                ...
                if cc and p["entry_price"]:
                    realized[st] += ((cc / p["entry_price"] - 1) - COSTS_ROUNDTRIP) * POSITION_PCT
            if r_wide and day in date_to_idx and date_to_idx[day] > 0:
                sig_fv, _ = _day_state_fv(per_ts, mv_map, list_dates, cal, cal[date_to_idx[day] - 1], "amplitude")
                fmap = sig_fv.get(st, {})
                if fmap and len(pos) < MAX_POSITIONS:
                    ...
                    for ts, open_px in cands[:slots]:
                        pos[ts] = {"entry_date": day, "entry_price": open_px}
            mtm = 0.0
            for ts, p in pos.items():
                cc = close_by_ts.get(ts, {}).get(day)
                mtm += POSITION_PCT * (cc / p["entry_price"]) if cc and p["entry_price"] else POSITION_PCT
            nav_state[st].append(1.0 + realized[st] + (mtm - len(pos) * POSITION_PCT))
    last = cal[-1]
    for st in STATE_FACTOR:
        for ts, p in list(positions[st].items()):
    ...
    for st in STATE_FACTOR:
        ...
        res[st] = {"total_pct": total_pct, "daily": total_pct / n if n else 0, "max_dd": max_dd, "sharpe": sharpe}
    return res
...
def main():
    ...
    out = {}
    print("\n### Per-state hold grid (valid daily / sharpe / dd) ###")
    for hold in HOLDS:
        print(f"\n--- hold={hold} ---")
        out[hold] = {}
        for w in WINDOWS:
            out[hold][w] = simulate_window(w, list_dates, hold)
        print("| state | OOS2 daily | train daily | valid daily | valid dd | sharpe |")
        print("|-------|-----------|-----------|-----------|---------|--------|")
        for st in STATE_FACTOR:
            o = out[hold]["OOS2"][st]; t = out[hold]["train"][st]; v = out[hold]["valid"][st]
            print(f"| {st:10s} | {o['daily']:+.4f} | {t['daily']:+.4f} | {v['daily']:+.4f} | {v['max_dd']:.1f} | {v['sharpe']:.2f} |")
    # best hold per state (valid daily, three-window positive)
    print("\n### Best hold per state (max valid daily, 3-window positive) ###")
    best = {}
    for st in STATE_FACTOR:
        best_h = None; best_d = -9
        for h in HOLDS:
            o = out[h]["OOS2"][st]["daily"]; t = out[h]["train"][st]["daily"]; v = out[h]["valid"][st]["daily"]
            if o > 0 and t > 0 and v > 0 and v > best_d:
                best_d = v; best_h = h
        best[st] = (best_h, best_d)
        print(f"{st:10s} -> hold {best_h} (valid {best_d:+.4f}/day)")
    p = REPORT_DIR / "state_hold_grid_latest.json"
    p.parent.mkdir(parents=True, exist_ok=True)
    p.write_text(json.dumps({"holds": HOLDS, "results": out, "best_per_state": {k: list(v) for k, v in best.items()}}, ensure_ascii=False, indent=2, default=str))
    print(f"report -> {p}")
