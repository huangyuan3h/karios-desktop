...
class TestLookupCn:
    def test_by_ticker(self, monkeypatch) -> None:
        monkeypatch.setattr(
            sr,
            "fetch_market_stocks",
            lambda **kw: (1, [{"ticker": "600519", "symbol": "600519", "name": "贵州茅台"}]),
        )
        ...
        monkeypatch.setattr(
            sr,
            "fetch_market_stocks",
            lambda **kw: (1, [{"ticker": "600519", "symbol": "CN:600519", "name": "贵州茅台"}]),
        )
        assert sr._lookup_by_ticker("600519")["symbol"] == "CN:600519"
        monkeypatch.setattr(
            sr,
            "fetch_market_stocks",
            lambda **kw: (1, [{"ticker": "000001", "symbol": "000001", "name": "平安银行"}]),
        )
        assert sr._lookup_by_ticker("600519") is None
        monkeypatch.setattr(
            sr, "fetch_market_stocks", lambda **kw: (1, [{"ticker": "", "symbol": "", "name": ""}])
        )
        assert sr._lookup_by_ticker("600519") is None
    ...
    def test_by_name(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            sr,
            "search_cn_candidates",
            lambda names, limit: [{"symbol": "CN:600519", "name": "贵州茅台"}],
        )
        ...
        monkeypatch.setattr(
            sr,
            "search_cn_candidates",
            lambda names, limit: [{"symbol": "600519", "ticker": "600519", "name": "贵州茅台"}],
        )
        out = sr._lookup_by_name("贵州茅台")
        assert out["symbol"] == "CN:600519"
    ...
    def test_by_name_ambiguous(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "data_sync_service.service.alpha_radar_qa.name_search_is_ambiguous", lambda **kw: True
        )
        out = sr._lookup_by_name("贵州茅台")
        assert out["confidence"] == 0.55
        assert out["rationale"].endswith("(ambiguous)")
        monkeypatch.setattr(
            "data_sync_service.service.alpha_radar_qa.name_search_is_ambiguous",
...
class TestLookupHk:
    def test_by_ticker(self, monkeypatch) -> None:
        monkeypatch.setattr(
            sr,
            "fetch_market_stocks",
            lambda **kw: (1, [{"ticker": "00700", "symbol": "00700", "name": "腾讯"}]),
        )
        ...
        monkeypatch.setattr(
            sr,
            "fetch_market_stocks",
            lambda **kw: (1, [{"ticker": "00700", "symbol": "HK:00700", "name": "腾讯"}]),
        )
        assert sr._lookup_hk_by_ticker("00700")["symbol"] == "HK:00700"
        monkeypatch.setattr(
            sr,
            "fetch_market_stocks",
            lambda **kw: (1, [{"ticker": "00941", "symbol": "00941", "name": "中移动"}]),
        )
    ...
    def test_by_name(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            sr,
            "fetch_market_stocks",
            lambda **kw: (1, [{"symbol": "00700", "ticker": "00700", "name": "腾讯"}]),
        )
        ...
        monkeypatch.setattr(
            sr,
            "fetch_market_stocks",
            lambda **kw: (1, [{"symbol": "HK:00700", "ticker": "00700", "name": "腾讯"}]),
        )
...
class TestResolve:
    ...
    def test_map_trend_hk(self, monkeypatch) -> None:
        monkeypatch.setattr(
            sr,
            "resolve_hk_mapping",
            lambda raw, logic_summary: (
                [{"symbol": "HK:00700", "name": "腾讯", "confidence": 0.85, "rationale": "r"}]
                if raw
                else [],
                ["x"],
            ),
        ...
        monkeypatch.setattr(
            sr,
            "update_trend_hk_mapping",
            lambda trend_id, hk_symbols: seen.update(trend_id=trend_id, hk_symbols=hk_symbols),
        )
        out = sr.map_trend_hk(
            trend_id="t1", trend={"hk_mapping": ["00700"], "logic_summary": "summary"}
        )
    ...
    def test_resolve_a_share(self, monkeypatch) -> None:
        ...
        resolved, unresolved = sr.resolve_a_share_mapping(
            ["600519", "600519", " ", "贵州茅台", "x"]
        )
        ...
        monkeypatch.setattr(
            ...
            "_lookup_by_name",
            lambda n: {
                "symbol": "CN:600519" if n == "贵州茅台" else "CN:300751",
                "name": n,
                "confidence": 0.75,
                "rationale": "n",
                "ambiguous": False,
    ...
    def test_map_trend_hybrid_local(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "data_sync_service.db.alpha_radar.update_trend_mapping", lambda **kw: None
        )
        monkeypatch.setattr(
            "data_sync_service.service.alpha_radar_risk.compute_risk_status", lambda **kw: "low"
        )
        out = sr.map_trend_hybrid(
            trend_id="t1",
            trend={
                "a_share_mapping": ["600519"],
                "macro_theme": "白酒",
                "keywords_for_mapping": ["茅台"],
            },
            hot_industry_names=["电子"],
            mainline_by_industry={},
        )
    ...
    def test_map_trend_hybrid_fallback(self, monkeypatch) -> None:
        monkeypatch.setattr(
            sr, "resolve_a_share_mapping", lambda raw, logic_summary: ([], ["贵州茅台"])
        )
        seen = {}
        monkeypatch.setattr(
            sr,
            ...
            lambda **kw: (
                seen.update(kw)
                or {"cnSymbols": [], "mappingConfidence": 0.5, "riskStatus": "medium", "ok": True}
            ),
        )
        out = sr.map_trend_hybrid(
            trend_id="t1", trend={"keywordsForMapping": ["贵州茅台"], "macroTheme": "白酒"}
        )
