...
def test_sync_macro_daily_full_driver(monkeypatch) -> None:
    ...
    monkeypatch.setattr(md, "get_last_trade_date", lambda sid: pd.Timestamp("2099-01-01").date())
...
def test_paged_index_global(monkeypatch) -> None:
    chunks = []
    pro = type(
        "P", (), {"index_global": staticmethod(lambda ts_code, start_date, end_date: chunks.pop(0))}
    )()
    ...
    pro2 = type(
        "P", (), {"index_global": staticmethod(lambda ts_code, start_date, end_date: None)}
    )()
    ...
    pro3 = type(
        ...
        (),
        {
            "index_global": staticmethod(
                lambda ts_code, start_date, end_date: (_ for _ in ()).throw(RuntimeError("x"))
            )
        },
    )()
...
def test_paged_fut_daily(monkeypatch) -> None:
    ...
    chunks = [d1]
    pro = type(
        "P", (), {"fut_daily": staticmethod(lambda ts_code, start_date, end_date: chunks.pop(0))}
    )()
...
def test_resolve_sgx_a50_main() -> None:
    ...
    pro2 = type(
        "P", (), {"fut_basic": staticmethod(lambda **kw: (_ for _ in ()).throw(RuntimeError("x")))}
    )()
...
def test_resolve_ine_sc_main_and_prefix(monkeypatch) -> None:
    df = pd.DataFrame(
        {
            "ts_code": ["SC2601.INE", "AU2606.SHFE", "CU2603.SHFE"],
            "name": ["原油", "金", "铜"],
            "list_date": ["2025-01-01", "2025-02-01", "2025-03-01"],
        }
    )
    pro = type("P", (), {"fut_basic": staticmethod(lambda **kw: df)})()
...
def test_sync_macro_daily_full_success(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        md,
        "get_settings",
        lambda: type("S", (), {"tu_share_api_key": "K", "tushare_tokens": ("K",)})(),
    )
    monkeypatch.setattr(md, "_tushare_pro", lambda: object())
    ...
    monkeypatch.setattr(
        md,
        "_paged_index_global",
        lambda pro, code, s, e: pd.DataFrame({"trade_date": ["2026-08-07"]}),
    )
    monkeypatch.setattr(
        md, "_normalize_fx_daily_df", lambda df: pd.DataFrame({"trade_date": ["2026-08-07"]})
    )
    ...
    monkeypatch.setattr(
        md, "_paged_fut_daily", lambda pro, code, s, e: pd.DataFrame({"trade_date": ["2026-08-07"]})
    )
...
def test_sync_macro_daily_full_resume_and_failure(monkeypatch) -> None:
    monkeypatch.setattr(
        md, "get_today_run", lambda job: {"success": False, "last_ts_code": md.SID_DJI}
    )
    monkeypatch.setattr(
        md,
        "get_settings",
    ...
    monkeypatch.setattr(
        md,
        "_paged_index_global",
        lambda pro, code, s, e: (_ for _ in ()).throw(RuntimeError("provider down")),
    )
    monkeypatch.setattr(md, "insert_record", lambda **kw: None)
    out = md.sync_macro_daily_full()
    assert out["ok"] is False
