...
def test_reconcile_day_hk_market(monkeypatch) -> None:
    ...
    with (
        patch.object(recon, "simulate", side_effect=[_fake_run(), _hk_run()]),
        patch.object(recon, "BacktestData", return_value=None),
    ):
        out = recon.reconcile_day("2026-08-07")
...
def test_reconcile_registry_extra_kinds(monkeypatch) -> None:
    ...
    with (
        patch.object(recon, "simulate", return_value=_fake_run_reg()),
        patch.object(recon, "BacktestData", return_value=None),
    ):
