...
class TestFundFlow:
    def test_get_ok(self, monkeypatch) -> None:
        monkeypatch.setattr(
            ifr, "get_cn_industry_fund_flow", lambda days, top_n, as_of_date: {"items": []}
        )
    ...
    def test_get_error(self, monkeypatch) -> None:
        monkeypatch.setattr(
            ...
            lambda days, top_n, as_of_date: (_ for _ in ()).throw(RuntimeError("boom")),
        )
    ...
    def test_sync_defaults(self, monkeypatch) -> None:
        ...
        assert (
            ifr.market_cn_industry_fund_flow_sync(ifr.IndustryFundFlowSyncRequest())["ok"] is True
        )
    ...
    def test_sync_custom(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(ifr, "sync_cn_industry_fund_flow", fake)
        ifr.market_cn_industry_fund_flow_sync(
            ifr.IndustryFundFlowSyncRequest(days=5, topN=20, force=True)
        )
    ...
    def test_sync_error(self, monkeypatch) -> None:
        monkeypatch.setattr(
            ...
            lambda days, top_n, force: (_ for _ in ()).throw(RuntimeError("boom")),
        )
        with pytest.raises(HTTPException) as exc:
...
class TestMainline:
    ...
    def test_get_error(self, monkeypatch) -> None:
        monkeypatch.setattr(
            ...
            lambda as_of_date: (_ for _ in ()).throw(ValueError("boom")),
        )
        with pytest.raises(HTTPException) as exc:
    ...
    def test_sync_ok(self, monkeypatch) -> None:
        ...
        ifr.market_cn_industry_mainline_sync(
            ifr.IndustryMainlineSyncRequest(asOfDate="2026-08-07", force=True)
        )
    ...
    def test_sync_error(self, monkeypatch) -> None:
        monkeypatch.setattr(
            ...
            lambda as_of_date, force: (_ for _ in ()).throw(RuntimeError("boom")),
        )
        with pytest.raises(HTTPException) as exc:
            ifr.market_cn_industry_mainline_sync(ifr.IndustryMainlineSyncRequest())
        assert exc.value.status_code == 500
