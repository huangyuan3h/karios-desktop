...
def test_build_attaches_candidates_and_pyramid_flags():
    ...
    with (
        ...
        patch(
            "data_sync_service.service.backtest_engine._load_regime_by_day",
            return_value={"2026-08-07": "Strong"},
        ),
        patch(
            "data_sync_service.service.market_sentiment.get_cn_sentiment",
            return_value={"items": [{"riskMode": "hot"}]},
        ),
        patch(
            "data_sync_service.service.market_sentiment.get_panic_cooldown",
    ...
    items = [
        {
            ...
            "eventFocus": "美联储降息预期升温",
            "documentPublishedAt": "2026-08-10T08:00:00+08:00",
            "createdAt": "x",
            "cnSymbols": [],
            "hkSymbols": [{"symbol": "HK:02099", "confidence": 0.9}],
        },
        {
            "trendName": "低置信事件",
            "catalystGrade": "C",
            ...
            "riskStatus": "risk",
            "eventFocus": "",
            "documentPublishedAt": None,
            "createdAt": "x",
            "cnSymbols": [{"symbol": "CN:300628", "confidence": 0.3}],
            "hkSymbols": [],
        },
        {
            "trendName": "通信设备景气",
            ...
            "riskStatus": "ok",
            "eventFocus": "5G 资本开支超预期",
            "documentPublishedAt": None,
            "createdAt": "2026-08-11T00:00:00Z",
            "cnSymbols": [{"symbol": "CN:300628", "confidence": 0.85}],
            "hkSymbols": [],
        },
    ]
    monkeypatch.setattr(
        "data_sync_service.db.alpha_radar.fetch_trends",
    ...
    for i, name in enumerate(("通信", "有色金属", "电子")):
        for d in dates:
            rows.append(
                {
                    "date": d,
                    "industry_code": f"BK{i}",
                    "industry_name": name,
                    "net_inflow": (100.0 - i * 20.0) * 1e8,
                }
            )
    monkeypatch.setattr(
        "data_sync_service.db.industry_fund_flow.get_dates_upto", lambda day, days: dates
    )
    monkeypatch.setattr(
        "data_sync_service.db.industry_fund_flow.get_rows_for_dates", lambda ds: rows
    )
...
def test_build_holdings_attaches_alpha_and_flow(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "data_sync_service.db.watchlist_automation.update_registry_payload", lambda *a, **k: None
    )
    monkeypatch.setattr(
        ph,
...
def test_health_block_candidate_info_and_summary(monkeypatch) -> None:
    ...
    monkeypatch.setattr(ph, "_pyramided_symbols", lambda: set())
    monkeypatch.setattr(ph, "_lookup_stock_basic", lambda ts: ({"600111.SH": "北方稀土"}, {}))
    ...
    monkeypatch.setattr(
        ...
        lambda days, cooldown_days, as_of_date: {
            "lastPanicDate": None,
            "cooldownEndDate": None,
            "active": False,
        },
    )
    monkeypatch.setattr(
        "data_sync_service.service.paper_s3.build_s3_candidates",
        lambda trade_date, max_positions: cands,
    )
    monkeypatch.setattr("data_sync_service.service.paper_s3._circuit_blocked", lambda as_of: False)
    monkeypatch.setattr(
        "data_sync_service.db.watchlist_automation.update_registry_payload", lambda *a, **k: None
    )
    ...
    assert c["industryFlow"] == {
        "industry": "有色金属",
        "netInflow5d": 8.2,
        "rank5d": 2,
        "total": 31,
    }
    assert out["infoSummary"] == {
        "holdingsCount": 1,
        "eventHoldings": 0,
        "industryOutflow": 1,
        "industryInflow": 0,
