...
def _bars(
    close: float, high: float, low: float, days: int = 30
...
def test_v5_alert_buy_add(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: {"000001.SZ": _bars(100, 100, 98)},
    )
...
def test_v5_alert_trim_in_weak_regime(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        alerts,
        "fetch_last_ohlcv_batch",
        lambda *_args, **_kwargs: {"000001.SZ": _bars(100, 130, 98)},
    )
...
def test_v5_plan_keeps_holdings_when_no_signal(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        alerts,
        "fetch_last_ohlcv_batch",
        lambda *_args, **_kwargs: {"000001.SZ": _bars(98, 120, 95)},
    )
    monkeypatch.setattr(alerts, "get_market_regime", lambda **_kwargs: {"regime": "Strong"})
