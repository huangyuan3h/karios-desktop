...
class TestEndpoints:
    ...
    def test_put_prompt(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            spr.spsvc,
            "put_system_prompt_value",
            lambda value: (_ for _ in ()).throw(HTTPException(status_code=400, detail="bad")),
        )
        with pytest.raises(HTTPException) as exc:
    ...
    def test_list_prompts(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            spr.spsvc, "list_presets", lambda: (_ for _ in ()).throw(RuntimeError("boom"))
        )
    ...
    def test_create_prompt(self, monkeypatch) -> None:
        ...
        assert create_system_prompt(CreateSystemPromptPresetRequest(title="t", content="c")) == {
            "id": "p1"
        }
        monkeypatch.setattr(
            spr.spsvc,
            "create_preset",
            lambda title, content: (_ for _ in ()).throw(RuntimeError("boom")),
        )
        with pytest.raises(HTTPException):
    ...
    def test_active(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            spr.spsvc, "get_active_prompt", lambda: (_ for _ in ()).throw(RuntimeError("boom"))
        )
        with pytest.raises(HTTPException):
            get_active_system_prompt()
        ...
        monkeypatch.setattr(
            spr.spsvc,
            "set_active_prompt",
            lambda preset_id: (_ for _ in ()).throw(RuntimeError("boom")),
        )
        with pytest.raises(HTTPException):
    ...
    def test_preset_crud(self, monkeypatch) -> None:
        monkeypatch.setattr(
            spr.spsvc, "get_preset", lambda preset_id: {"id": preset_id, "value": "v"}
        )
        assert get_system_prompt_preset("p1")["id"] == "p1"
        monkeypatch.setattr(
            spr.spsvc, "get_preset", lambda preset_id: (_ for _ in ()).throw(RuntimeError("boom"))
        )
        with pytest.raises(HTTPException):
            get_system_prompt_preset("p1")
        monkeypatch.setattr(
            spr.spsvc, "update_preset", lambda preset_id, title, content: {"ok": True}
        )
        assert put_system_prompt_preset(
            "p1", UpdateSystemPromptPresetRequest(title="t", content="c")
        ) == {"ok": True}
        monkeypatch.setattr(
            spr.spsvc,
            "update_preset",
            lambda preset_id, title, content: (_ for _ in ()).throw(RuntimeError("boom")),
        )
        with pytest.raises(HTTPException):
...
class TestHttpExceptionPassthrough:
    def test_all_endpoints_rethrow(self, monkeypatch) -> None:
        ...
        cases = [
            ...
            lambda: put_system_prompt_preset(
                "p1", UpdateSystemPromptPresetRequest(title="t", content="c")
            ),
