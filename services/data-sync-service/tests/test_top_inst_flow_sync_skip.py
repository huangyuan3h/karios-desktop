...
def test_sync_top_inst_uses_tushare_provider_result(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        svc, "upsert_daily_rows", lambda rows: inserted_daily_rows.append(rows) or len(rows)
    )
    monkeypatch.setattr(
        svc, "upsert_summary_rows", lambda rows: inserted_summary_rows.append(rows) or len(rows)
    )
    monkeypatch.setattr(svc, "insert_record", lambda **kwargs: None)
...
def test_sync_top_inst_skips_when_today_lhb_not_published(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        svc, "_is_today", lambda _td: True
    )  # 2026-08-21: frozen date no longer equals today
