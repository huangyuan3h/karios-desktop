...
def _run(bars, cn_block, *, last_date: date | None = None, holdings_override=None):
    with (
        patch(
            "data_sync_service.service.third_asset_sleeve.get_last_trade_date",
            return_value=last_date,
        ),
...
def test_sell_to_repo_when_below_ma200_and_holding():
    ...
    out = _run(
        _bars(last_close=80.0), _cn_block(), last_date=date(2026, 8, 6), holdings_override=holdings
    )
    assert out["active"] is True
...
def test_missing_local_bars_triggers_sync():
    with (
        patch(
            "data_sync_service.service.third_asset_sleeve.get_last_trade_date", return_value=None
        ),
        patch("data_sync_service.service.third_asset_sleeve.fetch_last_bars", return_value=_bars()),
        patch(
            "data_sync_service.service.etf_daily.sync_etf_daily_for_ts_code", return_value={}
        ) as sync,
    ):
...
def test_sleeve_for_paper_uses_paper_book():
    ...
    with (
        patch(
            "data_sync_service.service.portfolio_health._health_block",
            return_value=_cn_block(holdings=[]),
        ) as hb,
...
def test_resolve_held_third_asset_derives_ts_code() -> None:
    ...
    rows = [
        {"symbol": "ETF:513110", "positionPct": 23.61, "costPrice": 2.45, "entryDate": "2026-08-20"}
    ]
...
def test_build_third_asset_holding_hold_when_above_ma200() -> None:
    ...
    holdings = [
        {"symbol": "ETF:513110", "ts_code": "513110.SH", "positionPct": 23.61, "costPrice": 2.45}
    ]
    with patch(
        "data_sync_service.service.third_asset_sleeve._etf_market_data",
        return_value=_md(above=True),
    ):
        out = tas.build_third_asset_holding(
            day="2026-08-20", cn_block=_cn_block(regime="Diverging"), holdings_override=holdings
        )
...
def test_build_third_asset_holding_sell_when_below_ma200() -> None:
    ...
    with patch(
        "data_sync_service.service.third_asset_sleeve._etf_market_data",
        return_value=_md(above=False, close=2.1, ma200=2.4),
    ):
        out = tas.build_third_asset_holding(
            day="2026-08-20", cn_block=_cn_block(regime="Weak"), holdings_override=holdings
        )
    assert out is not None
    assert out["action"] == tas.ACTION_SELL_TO_REPO
    assert out["aboveMa200"] is False
...
def test_build_third_asset_holding_sell_when_a_share_buy_setup() -> None:
    ...
    with patch(
        ...
        out = tas.build_third_asset_holding(
            day="2026-08-20", cn_block=block, holdings_override=holdings
        )
    assert out is not None
    assert out["action"] == tas.ACTION_SELL_TO_A_SHARE
...
def test_build_third_asset_holding_none_when_not_held() -> None:
    out = tas.build_third_asset_holding(
        day="2026-08-20", cn_block=_cn_block(), holdings_override=[]
    )
    assert out is None
...
def test_cn_holdings_exclude_third_asset_etf() -> None:
    ...
    reg = [
        {
            "symbol": "ETF:513110",
            "payload": {"positionPct": 23.61, "costPrice": 2.45, "entryDate": "2026-08-20"},
        },
        {
            "symbol": "CN:300628",
            "payload": {"positionPct": 6.4, "costPrice": 39.9, "entryDate": "2026-08-04"},
        },
    ]
    with (
        ...
        patch.object(
            ph,
            "_holding_check",
            lambda **kw: {"symbol": kw["name"], "action": "HOLD", "pnlPct": 1.0},
        ),
