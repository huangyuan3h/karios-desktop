...
def _bars(n: int = 88, vol: float = 1e6) -> list[tuple[str, str, str, str, str, str]]:
    ...
    for i, c in enumerate(closes):
        ...
        bars.append(
            (dt, f"{c * 0.998:.3f}", f"{c * 1.004:.3f}", f"{c * 0.993:.3f}", f"{c:.3f}", f"{v:.0f}")
        )
    return bars
...
class TestHelpers:
    ...
    def test_get_regime(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            wma, "get_market_regime", lambda **kw: (_ for _ in ()).throw(RuntimeError("down"))
        )
    ...
    def test_latest_bar_date(self) -> None:
        ...
        assert (
            wma._latest_bar_date({"a": [], "b": [("2026-08-01",), ("2026-08-02",)]}) == "2026-08-02"
        )
    ...
    def test_merge_realtime_bar(self) -> None:
        bars = _bars(5)
        merged = wma._merge_realtime_bar(
            bars, {"price": "150.0", "trade_time": "2026-08-06 15:00:00", "volume": "5"}
        )
        assert len(merged) == 6 and merged[-1][4] == "150.0"
        same = wma._merge_realtime_bar(
            bars, {"price": "150.0", "trade_time": f"{bars[-1][0]} 15:00:00"}
        )
        assert len(same) == 5
...
class TestCompute:
    ...
    def test_full_breakout(self, monkeypatch) -> None:
        ...
        items = [
            {"symbol": "CN:600519", "position_pct": 0.05, "entry_price": 100.0, "max_price": 120.0}
        ]
    ...
    def test_hold_when_target_reached(self, monkeypatch) -> None:
        self._patch(monkeypatch, regime="Strong")
        items = [
            {"symbol": "CN:600519", "position_pct": 0.30, "entry_price": 100.0, "max_price": 120.0}
        ]
    ...
    def test_realtime_not_ok(self, monkeypatch) -> None:
        self._patch(monkeypatch)
        monkeypatch.setattr(wma, "fetch_realtime_quotes", lambda codes: {"ok": False})
        out = wma.compute_watchlist_momentum_alerts(
            [{"symbol": "CN:600519", "position_pct": 0.05}], realtime=True
        )
