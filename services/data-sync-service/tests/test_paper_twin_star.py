...
def test_intake_skips_when_slots_full(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        pts, "insert_paper_trade", lambda **k: (_ for _ in ()).throw(AssertionError("no insert"))
    )
    out = pts.run_intake_twin_star(trade_date="2026-09-02")
...
def test_intake_prefers_bar_1430_and_records_source(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        pts,
        "_fetch_1430_px_map",
        lambda ts, day: ({"000001.SZ": 10.2}, {"000001.SZ": "bar_5min_1430"}),
    )
...
def test_update_records_exit_source_and_hhmm(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        pts,
        "_fetch_1430_px_map",
        lambda ts, day: ({"000001.SZ": 10.1}, {"000001.SZ": "bar_5min_1430"}),
    )
