...
def _patch(
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: [
            d
            for d in (open_dates if open_dates is not None else [date(2026, 8, 7)])
    ...
    assert pro.daily_calls == [
        {
            ...
            "limit": 5000,
            "offset": 0,
            "fields": cs.DAILY_FIELDS_STR
            if hasattr(cs, "DAILY_FIELDS_STR")
            else ",".join(cs.DAILY_FIELDS),
        }
    ]
...
def test_resolve_start_date_last_success_time(monkeypatch) -> None:
    monkeypatch.setattr(
        cs,
        "get_last_success",
        lambda jt: {"last_ts_code": "", "sync_at": "2026-08-06T17:00:00+08:00"},
    )
    assert cs._resolve_start_date(date(2026, 8, 8), None) == date(2026, 8, 7)
...
def test_sync_close_force_reruns(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        cs, "_cn_now", lambda: datetime(2026, 8, 8, 18, 0, tzinfo=ZoneInfo("Asia/Shanghai"))
    )
...
def test_sync_close_too_early(monkeypatch) -> None:
    ...
    monkeypatch.setattr(cs, "_cn_today", lambda: date(2026, 8, 8))
    monkeypatch.setattr(
        cs, "_cn_now", lambda: datetime(2026, 8, 8, 10, 0, tzinfo=ZoneInfo("Asia/Shanghai"))
    )
    out = cs.sync_close()
...
def test_sync_close_success_trading_day(monkeypatch) -> None:
    pro = _patch(
        monkeypatch,
        open_flag=True,
        open_dates=[date(2026, 8, 7), date(2026, 8, 8)],
...
def test_sync_close_pre_close_catchup_partial(monkeypatch) -> None:
    ...
    monkeypatch.setattr(cs, "_cn_today", lambda: date(2026, 8, 8))
    monkeypatch.setattr(
        cs, "_cn_now", lambda: datetime(2026, 8, 8, 9, 0, tzinfo=ZoneInfo("Asia/Shanghai"))
    )
    out = cs.sync_close()
