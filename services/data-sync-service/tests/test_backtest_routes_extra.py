...
class TestRun:
    def test_run(self, monkeypatch) -> None:
        ...
        out = br.backtest_run(
            ...
            rs_rank_min=0.0,
            diverging_scale=0.0,
            drawdown_circuit_pct=0.0,
            panic_cooldown_days=0,
            slippage_pct=0.0,
            trend_score_min=0.0,
            exclude_boards="",
        )
    ...
    def test_run_config_error(self, monkeypatch) -> None:
        monkeypatch.setattr(
            br.BacktestConfig,
            "__init__",
            lambda self, **kw: (_ for _ in ()).throw(ValueError("bad config")),
        )
        with pytest.raises(HTTPException) as exc:
    ...
    def test_run_simulate_error(self, monkeypatch) -> None:
        monkeypatch.setattr(
            br, "simulate", lambda config: (_ for _ in ()).throw(RuntimeError("boom"))
        )
        with pytest.raises(HTTPException) as exc:
            br.backtest_run(
                start="2026-01-01",
...
class TestSensitivity:
    ...
    def test_error(self, monkeypatch) -> None:
        monkeypatch.setattr(
            br, "default_sensitivity_grid", lambda s, e: (_ for _ in ()).throw(RuntimeError("boom"))
        )
        with pytest.raises(HTTPException) as exc:
...
class TestWeeklyReview:
    ...
    def test_value_error(self, monkeypatch) -> None:
        monkeypatch.setattr(
            "data_sync_service.service.weekly_review.build_weekly_review",
            lambda end_date: (_ for _ in ()).throw(ValueError("bad date")),
        )
        with pytest.raises(HTTPException) as exc:
    ...
    def test_exception(self, monkeypatch) -> None:
        monkeypatch.setattr(
            "data_sync_service.service.weekly_review.build_weekly_review",
            lambda end_date: (_ for _ in ()).throw(RuntimeError("boom")),
        )
        with pytest.raises(HTTPException) as exc:
...
class TestCorrelation:
    def test_ok(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "data_sync_service.service.paper_trading._resolve_ts_code",
            lambda sym: ("CN:600519", "600519.SH"),
        )
        monkeypatch.setattr(
            "data_sync_service.service.correlation.em_industry_for_ts_code", lambda ts: "白酒"
        )
        monkeypatch.setattr(
            "data_sync_service.service.correlation.evaluate_correlation_cap",
            lambda positions, industries, include_matrix: {"overLimit": []},
        )
...
class TestOverview:
    def test_overview_reads_baseline_files(self, monkeypatch, tmp_path) -> None:
        ...
        reports.mkdir()
        (reports / "walk_forward_baseline.json").write_text(
            json.dumps(
                {
        ...
        (reports / "walk_forward_hk_baseline.json").write_text(
            json.dumps(
                {
        ...
        (reports / "rolling_oos_latest.json").write_text(
            json.dumps(
                {
...
class TestPaperVsBacktest:
    def test_ok(self, monkeypatch, tmp_path) -> None:
        (tmp_path / "paper_vs_backtest_latest.json").write_text(
            json.dumps(
                {
